...
- Define every cited key in `paperAbbreviations` in `docs/.vuepress/config/papers.ts`; this enables the visible dotted underline and hover explanation.
...
- For a paper already in the reading collection, use `[Link](/papers/<slug>/)`; the Markdown plugin chooses the current page language. Otherwise prefer its canonical paper page, DOI, conference page, or arXiv abstract. Run `npm run paper:config` after updates to synchronize links and ordering without dropping citation keys.
