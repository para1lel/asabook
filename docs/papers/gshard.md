...
扩展神经网络在各种机器学习问题上带来了显著的质量提升 [Aro18, Fra18, Kap20, Dev18, Mah18, Bro77]. 对于计算机视觉来说, 增加模型容量已经在多种计算机视觉架构中提高了图像分类和检测的准确性 [He16, He16a, Ghi19]. 同样地, 在自然语言处理领域, 扩展 Transformer [Vas17b] 在语言理解任务 [Dev18, Raf19, Bro77], 跨语言下游迁移 [Dev18, Con19] 以及 (大规模) 多语言神经机器翻译 [Ari19, Hua19, Sha17] 上也带来了持续的提升. 这一普遍趋势促使最近的研究审视在扩展成功中起关键作用的因素 [Adv17, Hes17, Hes19, Gei20, Kap20], 包括训练数据的数量, 模型大小以及过去研究所发现的计算量. 虽然最终模型的质量被发现与数据量, 计算量和模型大小呈幂律关系 [Hes17, Kap20], 但更大模型带来的显著质量提升也伴随着各种实际挑战. 其中最重要的之一是训练效率, 我们将其定义为为了实现优于现有最佳系统的模型质量而使用的计算量和训练时间, 而这一点往往被忽略.
...
**计算成本与模型规模的超线性增长.** 通过增加深度或宽度直接扩展模型规模 [Bro77, Hua19] 通常会导致训练步骤时间至少呈线性增加. 通过在多个设备上拆分层权重和计算实现模型并行通常是必要的, 这会导致网络通信开销和设备未充分利用. 设备未充分利用源于分配不均衡以及底层神经网络的顺序依赖. 这种计算成本与模型规模之间的超线性关系无法仅通过使用更多设备来解决, 从而使训练大规模模型变得不切实际.
