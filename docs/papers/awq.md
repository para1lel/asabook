...
基于转换器的大型语言模型 (LLMs) [Advand17] 在各种基准测试中表现出色 [Systea01, Open22, Touvrc23, Xivr22]. 然而, 模型体积大导致服务成本高. 例如, GPT-3 有 1750 亿参数, 以 FP16 存储时为 350GB, 而最新的 H100 GPU 内存仅为 96GB, 更不用说边缘设备了.
...
这里 $Q$ 表示权重量化函数 (例如, 使用组大小为 128 的 INT3/INT4 量化), $\mathbf{W}$ 是 FP16 格式的原始权重, $\mathbf{X}$ 是从小型校准集缓存的输入特征 (我们从预训练数据集中取一个小型校准集, 以避免对特定任务过拟合). $\mathbf{s}$ 是每个 (输入) 通道的缩放因子; 对于 $\mathbf{s^{-1}}\cdot\mathbf{X}$, 它通常可以与前一个操作 [Xivs22, Xivt22] 融合. 由于量化函数不可微, 我们无法直接使用普通反向传播来优化该问题. 有一些技术依赖于近似梯度 [Bengio13, Xivf02], 但我们发现仍然存在收敛不稳定的问题.
...
我们在 LLaMA [Touvrc23] 和 OPT [Open22] 系列上对我们的方法进行了基准测试. 还有其他开源 LLM, 例如 BLOOM [Xivr22], 但它们的质量普遍较差, 因此我们未将其纳入研究. 我们进一步对一个指令调优模型 Vicuna [Marcha23] 以及视觉语言模型 OpenFlamingo-9B [March23] 和 LLaVA-13B [Visual23] 进行了基准测试, 以展示我们方法的通用性.
...
我们的主要基线是普通的四舍五入量化 (RTN). 当使用像 128 这样的小组大小时, 它实际上表现相当强 [Xivo22, Xivn22]. 我们还比较了用于大语言模型权重量化的最先进方法 GPTQ [Xivo22]. 对于 GPTQ, 我们还比较了使用“重新排序”技巧的更新版本 (记作 GPTQ-Reorder 或 GPTQ-R). 其他技术如 ZeroQuant [Effici22], AdaRound [PMLRa20] 和 BRECQ [Xivaw21] 依赖反向传播来更新量化权重, 可能难以扩展到大模型; 它们也未能优于 GPTQ [Xivo22], 因此未纳入研究.

### 3.2 评估

#### LLaMA 模型结果

我们将研究重点放在 LLaMA 模型 (LLaMA [Touvrc23] 和 Llama-2 [Xivf23]) 上, 因为它们相比其他开源大语言模型表现更优 [Open22, Xivr22]; 它也成为许多流行开源模型的基础 [Stanfa23, Marcha23]. 我们在量化前后评估了困惑度, 见[表 4](#table-04). 可以看到, AWQ 在不同模型规模 (7B-70B) 和生成任务中, 始终优于四舍五入 (RTN) 和 GPTQ [Xivo22] (有或没有重新排序).
...
量化降低了深度学习模型的位精度 [ICLRb16, Recogo18, Visiod19, CVPRc19, PMLRa20, Systeq20], 这有助于减小模型大小并加速推理. 量化技术通常分为两类: 量化感知训练 (QAT, 依赖反向传播更新量化权重) [Bengio13, Xivav21, Xivax21, Xivan18] 和训练后量化 [Recogo18, Visiod19, PMLRa20] (PTQ, 通常无需训练). QAT 方法难以轻松扩展到像 LLMs 这样的大模型. 因此, 人们通常使用 PTQ 方法对 LLMs 进行量化.

#### LLM 的量化.

人们研究了 LLM 量化的两种设置: (1) W8A8 量化, 其中激活和权重都量化为 INT8 [Xivm22, Xivt22, Effici22, Pushin22, Xivg23]; (2) 仅低位权重量化 (例如 W4A16), 其中仅将权重量化为低位整数 [Xivo22, Xivn22, Xivd23, Xivq22]. 本文重点研究第二种设置, 因为它不仅降低了硬件门槛 (需要更小的内存大小), 也加快了令牌生成速度 (缓解了内存受限的工作负载). 除了常规的四舍五入基线 (RTN) 之外, GPTQ [Xivo22] 最接近我们的工作. 然而, GPTQ 的重建过程会导致对校准集的过拟合问题, 而且可能无法保留 LLMs 在其他模态和领域中的通用能力. 它还需要一个重新排序技巧才能在某些模型上工作 (例如 LLaMA-7B [Touvrc23] 和 OPT-66B [Open22]).
