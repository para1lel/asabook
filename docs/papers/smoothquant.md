...
如 [图 3](#figure-03) 所示, 量化具有不同的粒度层次. 每张量量化为整个矩阵使用一个量化步长. 我们可以通过对每个令牌相关的激活使用不同的量化步长 (每令牌量化) 或对权重的每个输出通道使用不同的量化步长 (每通道量化) 来进一步启用更细粒度的量化. 每通道量化的粗粒度版本是对不同的通道组使用不同的量化步长, 称为分组量化 [Intela20, Yao22].

对于 Transformers 中的线性层 [Advane17] $\mathbf{Y}=\mathbf{X}\cdot\mathbf{W},\mathbf{Y}\in\mathbb{R}^{T\times C_{o}},\mathbf{X}\in\mathbb{R}^{T\times C_{i}},\mathbf{W}\in\mathbb{R}^{C_{i}\times C_{o}}$, 其中 $T$ 是令牌数, $C_{i}$ 是输入通道数, $C_{o}$ 是输出通道数 (见 [图 3](#figure-03), 为了简化省略批次维度), 通过将权重量化为 INT8, 我们可以将存储量相比 FP16 减少一半. 然而, 为了加快推理速度, 我们需要将权重和激活都量化为 INT8 (即 W8A8), 以利用整数核 (例如 INT8 GEMM), 这些在广泛的硬件上都受到支持 (例如 NVIDIA GPUs, Intel CPUs, Qualcomm DSPs 等).
...
1\. 激活比权重更难量化. 权重分布相当均匀和平坦, 这容易量化. 先前的工作表明, 即使使用 INT8 甚至 INT4 对 LLM 的权重进行量化也不会降低准确性 [Dettma22, Yao22, Xivk22], 这与我们的观察一致.
...
我们发现对于大多数模型, 例如所有 OPT [Zha22] 和 BLOOM [Scao22] 模型, $\alpha=0.5$ 是一个平衡点, 可以均匀地分配量化难度, 特别是在我们对权重和激活使用相同量化器 (例如每张量, 静态量化) 时. 该公式确保对应通道的权重和激活具有相似的最大值, 从而共享相同的量化难度. [图 5](#figure-05) 展示了当我们采用 $\alpha=0.5$ 时的平滑转换. 对于一些激活异常值更显著的其他模型 (例如 GLM-130B [Xivk22] 有 $\sim$ 30%的异常值, 这些对激活量化更困难), 我们可以选择更大的 $\alpha$ 将更多的量化难度迁移到权重 (如 0.75).
...
我们选择三大家族的 LLM 来评估 SmoothQuant: OPT [Zha22], BLOOM [Scao22] 和 GLM-130B [Xivk22]. 我们使用七个零样本评估任务: LAMBADA [August16], HellaSwag [Zeller19], PIQA [Bisk20], WinoGrande [Sakagu19], OpenBookQA [EMNLPb18], RTE [Wanga18], COPA [Papers11], 以及一个语言建模数据集 WikiText [Merity16] 来评估 OPT 和 BLOOM 模型. 我们使用 MMLU [Hen20], MNLI [Bowman18], QNLI [Wanga18] 和 LAMBADA 来评估 GLM-130B 模型, 因为上述一些基准在 GLM-130B 的训练集中出现过. 我们使用 lm-eval-harness [+1] 来评估 OPT 和 BLOOM 模型, 并使用 GLM-130B 的官方仓库 [+2] 进行其自身评估. 最后, 我们将方法扩展到 MT-NLG 530B [Smith22], 并首次实现了在单个节点上提供服务大于 500B 模型. 注意, 我们关注的是量化前后*相对*性能变化, 而非绝对值.

#### 激活平滑.

迁移强度 $\alpha=0.5$ 是所有 OPT 和 BLOOM 模型的通用最佳点, 而 GLM-130B 的 $\alpha=0.75$ 则更适合, 因为它的激活值更难量化 [Xivk22]. 通过在 Pile [Gao20] 验证集的一个子集上进行快速网格搜索, 我们可以得到合适的 $\alpha$. 为了获得激活统计数据, 我们使用预训练数据集 Pile 中的 512 个随机句子 *一次* 校准平滑因子和静态量化步长, 并将相同的平滑和量化模型应用于所有下游任务. 通过这种方式, 我们可以对量化的 LLM 的通用性和零样本性能进行基准测试.
...
预训练语言模型通过*扩展规模*在各种基准测试中取得了显著的表现. GPT-3 [Brownc20] 是第一个参数量超过 1000 亿的 LLM, 并取得了令人印象深刻的少样本/零样本学习结果. 后续的工作 [Rae21, Smith22, PMLR22, Chowdh22] 继续推动规模化的前沿, 参数量突破了 5000 亿. 然而, 随着语言模型规模的增大, 为此类模型提供推理服务变得昂贵且具有挑战性. 在本工作中, 我们展示了所提出的方法能够对三种最大的公开可用 LLM 进行量化: OPT-175B [Zha22], BLOOM-176B [Scao22] 和 GLM-130B [Xivk22], 甚至 MT-NLG 530B [Smith22], 从而降低内存成本并加速推理.

#### 模型量化.

量化是一种有效的减小模型大小和加速推理的方法. 它已被证明对各种卷积神经网络 (CNN) [Hana16, Jacob18, Nagel19, CVPRb19, Lina20] 和 Transformer [Intela20, Kim21, Liu21, Wanga20, Bondaa21] 都有效. 权重均衡 [Nagel19] 和通道分割 [PMLRb19] 通过抑制权重中的异常值来减少量化误差. 然而, 这些技术无法解决激活异常值问题, 而激活异常值是 LLM [Dettma22] 量化的主要瓶颈.
...
GPTQ 在处理少量输入 token (其实验中为 1 个 token) 时可能表现更好, 因为该过程高度受内存限制. 相比之下, SmoothQuant 在批量设置或上下文阶段 (即处理的 token 数量较多时) 可能表现更好. 然而, 一些研究表明, 在生产环境中, 我们可以通过先进的批处理 [OSDI22] 在相似延迟下将 GPT 模型的服务吞吐量提高 37$\times$. 我们相信, 在生产环境中, 批处理将成为未来的标准, 而 SmoothQuant 将带来进一步改进, 即使在生成阶段也是如此.
