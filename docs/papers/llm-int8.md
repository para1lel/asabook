...
大型预训练语言模型在自然语言处理 (NLP) 中被广泛采用 [Vas17c, Rad19a, Bro77, Zha22], 但在推理时需要大量内存. 对于参数量在 6.7B 及以上的大型 Transformer 语言模型, 前馈层和注意力投影层及其矩阵乘法操作占用了 95% [+1] 的参数, 并消耗了 65-85% 的计算量 [Ilh20]. 减少参数大小的一种方法是将它们量化为更少的位数, 并使用低位精度矩阵乘法. 基于这一目标, 已经开发了针对 Transformer 的 8 位量化方法 [Che20, Lin20a, Zaf19, She20]. 虽然这些方法可以减少内存使用, 但会降低性能, 通常需要在训练后进一步调节量化, 并且只针对参数少于 3.5 亿的模型进行了研究. 对于参数量高达 3.5 亿的无性能下降量化仍然理解不足, 而多十亿参数的量化仍是一个未解决的挑战.
