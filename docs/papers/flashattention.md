...
计算受限: 操作所需的时间由算术操作的数量决定, 而访问 HBM 的时间要小得多. 典型的例子是内维度较大的矩阵乘法和通道数较多的卷积运算.

2.2.

内存受限: 操作所需的时间由内存访问次数决定, 而计算所花费的时间要小得多. 例子包括大多数其他操作: 按元素操作 (例如, 激活, dropout) 和归约操作 (例如, 求和, softmax, 批量归一化, 层归一化).
...
0: 矩阵 $\mathbf{Q},\mathbf{K},\mathbf{V}\in\mathbb{R}^{N\times d}$ 在 HBM 中.

1: 从 HBM 分块加载 $\mathbf{Q},\mathbf{K}$, 计算 $\mathbf{S}=\mathbf{Q}\mathbf{K}^{\top}$, 将 $\mathbf{S}$ 写入 HBM.

2: 从 HBM 读取 $\mathbf{S}$, 计算 $\mathbf{P}=\mathrm{softmax}(\mathbf{S})$, 将 $\mathbf{P}$ 写入 HBM.

3: 从 HBM 分块加载 $\mathbf{P}$ 和 $\mathbf{V}$, 计算 $\mathbf{O}=\mathbf{P}\mathbf{V}$, 将 $\mathbf{O}$ 写入 HBM.
...
因此, 如果我们跟踪一些额外的统计信息 ($m(x),\ell(x)$), 我们可以一次计算一块的 softmax. [+2] 因此我们将输入 $\mathbf{Q},\mathbf{K},\mathbf{V}$ 分成块 ([算法 1](#alg1) 第 [3] 行 (#alg1.l3 "在算法 1 ‣ 3.1 带平铺和重新计算的高效注意力算法 ‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 快速且内存高效的精确注意力, 具备 IO 感知")), 计算 softmax 值及额外统计信息 ([算法 1](#alg1) 第 [10] 行 (#alg1.l10 "在算法 1 ‣ 3.1 带平铺和重新计算的高效注意力算法 ‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 快速且内存高效的精确注意力, 具备 IO 感知")), 并合并结果 ([算法 1](#alg1) 第 [12] 行 (#alg1.l12 "在算法 1 ‣ 3.1 带平铺和重新计算的高效注意力算法 ‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 快速且内存高效的精确注意力, 具备 IO 感知")).
...
0: 在 HBM 中的矩阵 $\mathbf{Q},\mathbf{K},\mathbf{V}\in\mathbb{R}^{N\times d}$, 片上 SRAM 大小为 $M$.
...
3: 将 $\mathbf{Q}$ 分成 $T_{r}=\left\lceil\frac{N}{B_{r}}\right\rceil$ 个块 $\mathbf{Q}_{1},\dots,\mathbf{Q}_{T_{r}}$, 每块大小为 $B_{r}\times d$, 并将 $\mathbf{K},\mathbf{V}$ 分成 $T_{c}=\left\lceil\frac{N}{B_{c}}\right\rceil$ 个块 $\mathbf{K}_{1},\dots,\mathbf{K}_{T_{c}}$ 和 $\mathbf{V}_{1},\dots,\mathbf{V}_{T_{c}}$, 每块大小为 $B_{c}\times d$.

4: 将 $\mathbf{O}$ 分成 $T_{r}$ 块, 每块 $\mathbf{O}_{i},\dots,\mathbf{O}_{T_{r}}$, 大小为 $B_{r}\times d$, 将 $\ell$ 分成 $T_{r}$ 块, 每块 $\ell_{i},\dots,\ell_{T_{r}}$, 大小为 $B_{r}$, 将 $m$ 分成 $T_{r}$ 块, 每块 $m_{1},\dots,m_{T_{r}}$, 大小为 $B_{r}$.

5: 对 $1\leq j\leq T_{c}$ 执行

6: 从 HBM 将 $\mathbf{K}_{j},\mathbf{V}_{j}$ 加载到片上 SRAM.

7: 对 $1\leq i\leq T_{r}$ 执行

8: 从 HBM 将 $\mathbf{Q}_{i},\mathbf{O}_{i},\ell_{i},m_{i}$ 加载到片上 SRAM.

9: 在芯片上, 计算 $\mathbf{S}_{\mathrm{ij}}=\mathbf{Q}_{i}\mathbf{K}_{j}^\top\in\mathbb{R}^{B_{r}\times B_{c}}$.

10: 在芯片上, 计算 $\tilde{m}_{\mathrm{ij}}=\mathrm{rowmax}(\mathbf{S}_{\mathrm{ij}})\in\mathbb{R}^{B_{r}}$, $\tilde{\mathbf{P}}_{\mathrm{ij}}=\exp(\mathbf{S}_{\mathrm{ij}}-\tilde{m}_{\mathrm{ij}})\in\mathbb{R}^{B_{r}\times B_{c}}$ (逐点) 和 $\tilde{\ell}_{\mathrm{ij}}=\mathrm{rowsum}(\tilde{\mathbf{P}}_{\mathrm{ij}})\in\mathbb{R}^{B_{r}}$.

11: 在芯片上, 计算 $m_{i}^{\mathrm{new}}=\max(m_{i},\tilde{m}_{\mathrm{ij}})\in\mathbb{R}^{B_{r}}$, $\ell_{i}^{\mathrm{new}}=e^{m_{i}-m_{i}^{\mathrm{new}}}\ell_{i}+e^{\tilde{m}_{\mathrm{ij}}-m_{i}^{\mathrm{new}}}\tilde{\ell}_{\mathrm{ij}}\in\mathbb{R}^{B_{r}}$.

12: 将 $\mathbf{O}_{i}\leftarrow\mathrm{diag}(\ell_{i}^{\mathrm{new}})^{-1}(\mathrm{diag}(\ell_{i})e^{m_{i}-m_{i}^{\mathrm{new}}}\mathbf{O}_{i}+e^{\tilde{m}_{\mathrm{ij}}-m_{i}^{\mathrm{new}}}\tilde{\mathbf{P}}_{\mathrm{ij}}\mathbf{V}_{j})$ 写入 HBM.

13: 将 $\ell_{i}\leftarrow\ell_{i}^{\mathrm{new}}$, $m_{i}\leftarrow m_{i}^{\mathrm{new}}$ 写入 HBM.
...
证明的主要思想是, 给定 SRAM 大小为 $M$, 我们可以 $\Theta(M)$ $\mathbf{K},\mathbf{V}$ 每个 ([算法 1](#alg1“在 3.1 中, 一个带铺砖和重计算的高效注意力算法‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 带 IO 意识的快速且内存高效的精确关注”) 行[6](#alg1.l6 “在算法 1 ‣ 3.1 具有铺砖和重新计算的高效注意力算法中 ‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 快速且内存高效的精确注意力, 具备 IO 意识) ). 对于每个 $\mathbf{K}$ 和 $\mathbf{V}$ 块, 我们遍历所有 $\mathbf{Q}$ 块 ([算法 1](#alg1“在 3.1 中, 一种带铺砖和重计算的高效注意力算法‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 快速且内存高效的精确关注与 IO 意识”) 行[8](#alg1.l8 “在算法 1 ‣ 3.1 具有铺砖和重新计算的高效注意力算法中 ‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 带 IO 意识的快速且内存高效的精确关注“) ) 计算中间值, 从而 $\Theta(\mathrm{NdM}^{-1})$ 次通过 $\mathbf{Q}$. 每次通行加载 $\Theta(\mathrm{Nd})$ 元素, 相当于 $\Theta(N^{2}d^{2}M^{-1})$ 次 HBM 访问. 我们同样证明, 标准注意力的后向传递需要 $\Theta(\mathrm{Nd}+N^{2})$ 次 HBM 访问, 而 FlashAttention 的后向传递需要 $\Theta(N^{2}d^{2}M^{-1})$ 次 HBM 访问 ([附录 B](#A2 “附录 B 算法细节‣ FlashAttention: 快速且内存高效的精确关注与 IO 意识”) ).
...
其中 $(\mathbf{S}\odot\vmathbb{1}_{\tilde{\mathbf{M}}})_{\mathrm{kl}}=\mathbf{S}_{\mathrm{kl}}$ 如果 $\tilde{\mathbf{M}}_{\mathrm{kl}}=1$, 并且 $-\infty$ 如果 $\mathbf{M}_{\mathrm{kl}}=0$. 我们要求 $\tilde{\mathbf{M}}$ 具有块状形式: 对于某些块大小 $B_{r},B_{c}$, 对于所有 $k,l$, $\tilde{\mathbf{M}}_{k,l}=\mathbf{M}_{\mathrm{ij}}$ 具有 $i=\lfloor k/B_{r}\rfloor,j=\lfloor l/B_{c}\rfloor$, 且存在某些 $\mathbf{M}\in\{0,1\}^{N/B_{r}\times N/B_{c}}$.
...
我们看到, 应用块稀疏会通过稀疏性直接改善 IO 复杂度中较大的项. 对于大序列长度 $N$, $s$ 通常设置为 $N^{-1/2}$ [Child19] 或 $N^{-1}\log N$ [Zaheer20, Beltag20, Daoa22], 从而导致 $\Theta(N\sqrt{N})$ 或 $\Theta(N\log N)$ IO 复杂度. 在下游实验中, 我们使用固定的蝴蝶稀疏模式 [Daoa22], 已显示能够近似任意稀疏 [Dao20].
...
|GPT-2 中型 - Huggingface [Wolf20]|14.2|21.0 天 (1.0$\times$) ZX0030QXZ GPT-2 中型 - Megatron-LM [Shoeyb19]|
...
我们的实现以 Apex 的 FMHA 代码 ([https://github.com/NVIDIA/apex/tree/master/apex/contrib/csrc/fmha](https://github.com/NVIDIA/apex/tree/master/apex/contrib/csrc/fmha]) 为起点. 我们感谢 Young-Jun Ko 对其 FMHA 实现的深入讲解, 以及他对我们有关 CUDA 的问题所提供的详尽解答. 我们感谢 Sabri Eyuboglu, Megan Leszczynski, Laurel Orr, Yuhuai Wu, Beidi Chen 和 Xun Huang 对论文早期稿件提出的建设性反馈和建议. 我们也感谢 Markus Rabe 和 Charles Staats 对其注意力算法的有益讨论.
...
为了简单起见, 我们在此省略 softmax 过程中的最大值偏移步骤. [Section B. 3](#A2.SS3) 中的完整算法包含所有步骤.
...
我们有 $S_{\mathrm{ij}}=q_{i}^\topk_{j}$, 其中 $q_{i}$ 和 $k_{j}$ 分别是 $i$ 和 $j$ 的 $\mathbf{Q}$ 列和 $\mathbf{K}$ 列. 定义 softmax 的归一化常数:
...
我们看到, 一旦计算出 $L_{i}$, 我们可以通过反复求和 $\frac{e^{q_{i}^\topk_{j}}}{L_{i}}v_{j}$ 来在不使用额外内存的情况下计算 $o_{i}$. 因此前向传播可以用 $O(n)$ 额外内存计算:

1.1.

根据[公式 1](#A2.E1), 为所有 $i$ 计算 $L_{i}$, 这需要额外的 $O(n)$ 内存.

2.2.

根据[公式 2](#A2.E2), 为所有 $i$ 计算 $o_{i}$, 这需要额外的 $O(d)$ 内存.
...
回忆 $P_{i:}=\mathrm{softmax}(S_{i:})$. 利用 $y=\mathrm{softmax}(x)$ 的雅可比矩阵是 $\mathrm{diag}(y)-\mathrm{yy}^\top$ 这一事实, 我们得到
...
现在我们可以得到梯度 $\mathbf{\mathrm{dQ}}$ 和 $\mathbf{\mathrm{dK}}$. 回想一下 $S_{\mathrm{ij}}=q_{i}^\topk_{j}$, 所以
...
根据 [Eq. 3](#A2.E3) 计算所有 $j$ 的 $\mathrm{dv}_{j}$, 这需要 $O(d)$ 的额外内存.

2.2.

根据[式 4](#A2.E4), 为所有 $i$ 计算 $D_{i}$, 这将占用 $O(n)$ 额外内存.

3.3.

根据 [Eq. 5](#A2.E5) 计算所有 $i$ 的 $\mathrm{dq}_{i}$, 这需要 $O(d)$ 的额外内存.

4.4.

根据 [Eq. 6](#A2.E6) 计算所有 $j$ 的 $\mathrm{dk}_{j}$, 这需要 $O(d)$ 的额外内存.
...
0: 在 HBM 中的矩阵 $\mathbf{Q},\mathbf{K},\mathbf{V}\in\mathbb{R}^{N\times d}$, 尺寸为 $M$ 的片上 SRAM, softmax 缩放常数 $\tau\in\mathbb{R}$, 掩码函数 mask, dropout 概率 $p_{\mathrm{drop}}$.

1: 初始化伪随机数生成器状态 ${\cal R}$ 并保存到 HBM.

2: 设置块大小 $B_{c}=\left\lceil\frac{M}{4d}\right\rceil,B_{r}=\min\left(\left\lceil\frac{M}{4d}\right\rceil,d\right)$.

3: 在 HBM 中初始化 $\mathbf{O}=(0)_{N\times d}\in\mathbb{R}^{N\times d},\ell=(0)_{N}\in\mathbb{R}^{N},m=(-\infty)_{N}\in\mathbb{R}^{N}$.

4: 将 $\mathbf{Q}$ 分成 $T_{r}=\left\lceil\frac{N}{B_{r}}\right\rceil$ 块, 每块大小为 $B_{r}\times d$, 然后将 $\mathbf{K},\mathbf{V}$ 分成 $T_{c}=\left\lceil\frac{N}{B_{c}}\right\rceil$ 块 $\mathbf{K}_{1},\dots,\mathbf{K}_{T_{c}}$ 和 $\mathbf{V}_{1},\dots,\mathbf{V}_{T_{c}}$, 每块大小为 $B_{c}\times d$.

5: 将 $\mathbf{O}$ 分成 $T_{r}$ 块, 每块大小为 $B_{r}\times d$, 将 $\ell$ 分成 $T_{r}$ 块, 每块大小为 $B_{r}$, 将 $m$ 分成 $T_{r}$ 块, 每块大小为 $B_{r}$.

6: 对于 $1\leq j\leq T_{c}$ 执行

7: 将 $\mathbf{K}_{j},\mathbf{V}_{j}$ 从 HBM 加载到片上 SRAM.

8: 对于 $1\leq i\leq T_{r}$ 执行

9: 将 $\mathbf{Q}_{i},\mathbf{O}_{i},\ell_{i},m_{i}$ 从 HBM 加载到片上 SRAM.

10: 在芯片上, 计算 $\mathbf{S}_{\mathrm{ij}}=\tau\mathbf{Q}_{i}\mathbf{K}_{j}^\top\in\mathbb{R}^{B_{r}\times B_{c}}$.

11: 在芯片上, 计算 $\mathbf{S}_{\mathrm{ij}}^{\mathrm{masked}}=\mathrm{mask}(\mathbf{S}_{\mathrm{ij}})$.

12: 在芯片上, 计算 $\tilde{m}_{\mathrm{ij}}=\mathrm{rowmax}(\mathbf{S}_{\mathrm{ij}}^{\mathrm{masked}})\in\mathbb{R}^{B_{r}}$, $\tilde{\mathbf{P}}_{\mathrm{ij}}=\exp(\mathbf{S}_{\mathrm{ij}}^{\mathrm{masked}}-\tilde{m}_{\mathrm{ij}})\in\mathbb{R}^{B_{r}\times B_{c}}$ (逐点) , $\tilde{\ell}_{\mathrm{ij}}=\mathrm{rowsum}(\tilde{\mathbf{P}}_{\mathrm{ij}})\in\mathbb{R}^{B_{r}}$.

13: 在芯片上, 计算 $m_{i}^{\mathrm{new}}=\max(m_{i},\tilde{m}_{\mathrm{ij}})\in\mathbb{R}^{B_{r}}$, $\ell_{i}^{\mathrm{new}}=e^{m_{i}-m_{i}^{\mathrm{new}}}\ell_{i}+e^{\tilde{m}_{\mathrm{ij}}-m_{i}^{\mathrm{new}}}\tilde{\ell}_{\mathrm{ij}}\in\mathbb{R}^{B_{r}}$.

14: 在芯片上, 计算 $\tilde{\mathbf{P}}_{\mathrm{ij}}^{\mathrm{dropped}}=\mathrm{dropout}(\tilde{\mathbf{P}}_{\mathrm{ij}},p_{\mathrm{drop}})$.

15: 将 $\mathbf{O}_{i}\leftarrow\mathrm{diag}(\ell_{i}^{\mathrm{new}})^{-1}(\mathrm{diag}(\ell_{i})e^{m_{i}-m_{i}^{\mathrm{new}}}\mathbf{O}_{i}+e^{\tilde{m}_{\mathrm{ij}}-m_{i}^{\mathrm{new}}}\tilde{\mathbf{P}}_{\mathrm{ij}}^{\mathrm{dropped}}\mathbf{V}_{j})$ 写入 HBM.

16: 将 $\ell_{i}\leftarrow\ell_{i}^{\mathrm{new}}$, $m_{i}\leftarrow m_{i}^{\mathrm{new}}$ 写入 HBM.

17: 结束 for 循环

18: 结束 for 循环

19: 返回 $\mathbf{O},\ell,m,{\cal R}$.
...
0: 矩阵 $\mathbf{Q},\mathbf{K},\mathbf{V},\mathbf{\mathrm{dO}}\in\mathbb{R}^{N\times d}$, $\mathbf{P}\in\mathbb{R}^{N\times N}$ 在 HBM 中.

1: 从 HBM 按块加载 $\mathbf{P},\mathbf{\mathrm{dO}}$, 计算 $\mathbf{\mathrm{dV}}=\mathbf{P}^{\top}\mathbf{\mathrm{dO}}\in\mathbb{R}^{N\times d}$, 将 $\mathbf{\mathrm{dV}}$ 写入 HBM.

2: 从 HBM 按块加载 $\mathbf{\mathrm{dO}},\mathbf{V}$, 计算 $\mathbf{\mathrm{dP}}=\mathbf{\mathrm{dO}}\mathbf{V}^{\top}\in\mathbb{R}^{N\times N}$, 将 $\mathbf{\mathrm{dP}}$ 写入 HBM.

3: 从 HBM 读取 $\mathbf{P},\mathbf{\mathrm{dP}}$, 计算 $\mathbf{\mathrm{dS}}\in\mathbb{R}^{N\times N}$, 其中 $\mathrm{dS}_{\mathrm{ij}}=P_{\mathrm{ij}}(\mathrm{dP}_{\mathrm{ij}}-\sum_{l}P_{\mathrm{il}}\mathrm{dP}_{\mathrm{il}})$, 将 $\mathbf{\mathrm{dS}}$ 写入 HBM.

4: 从 HBM 按块加载 $\mathbf{\mathrm{dS}}$ 和 $\mathbf{K}$, 计算 $\mathbf{\mathrm{dQ}}=\mathbf{\mathrm{dS}}\mathbf{K}$, 将 $\mathbf{\mathrm{dQ}}$ 写入 HBM.

5: 从 HBM 按块加载 $\mathbf{\mathrm{dS}}$ 和 $\mathbf{Q}$, 计算 $\mathbf{\mathrm{dK}}=\mathbf{\mathrm{dS}}^{\top}\mathbf{Q}$, 将 $\mathbf{\mathrm{dK}}$ 写入 HBM.

6: 返回 $\mathbf{\mathrm{dQ}},\mathbf{\mathrm{dK}},\mathbf{\mathrm{dV}}$.

现在我们对 FlashAttention 反向传播提出两点观察:

1.1.

我们不需要存储前向传播中大小为 $O(N^{2})$ 的 dropout 掩码. 相反, 我们可以保存前向传播中的伪随机数生成器状态, 并在反向传播中重新生成 dropout 掩码. 这只需使用 $O(N)$ 的额外内存.

2.2.

在计算 softmax 梯度时, 我们使用 [Eq. 4](#A2.E4) 来计算 $D_{i}=P_{i:}^{\top}\mathrm{dP}_{i:}$, 而不对大小为 $N$ 的 $P_{i:}$ 和 $\mathrm{dP}_{i:}$ 进行压缩 (它们可能无法放入 SRAM). 相反, 我们可以重写 $D_{i}=\mathrm{do}_{i}^{\top}o_{i}$, 并计算大小为 $d$ 的向量之间的点积.

完整的 FlashAttention 反向传播算法在 [算法 4](#alg4) 中. 概念上, 它只是 [第 B. 2 节](#A2.SS2) 推导的块版本.

算法 4 FlashAttention 反向传播

0: HBM 中的矩阵 $\mathbf{Q},\mathbf{K},\mathbf{V},\mathbf{O},\mathbf{\mathrm{dO}}\in\mathbb{R}^{N\times d}$, HBM 中的向量 $\ell,m\in\mathbb{R}^{N}$, 大小为 $M$ 的芯片上 SRAM, softmax 缩放常数 $\tau\in\mathbb{R}$, 掩码函数 mask, dropout 概率 $p_{\mathrm{drop}}$, 来自前向传播的伪随机数生成器状态 ${\cal R}$.

1: 将伪随机数生成器状态设置为 ${\cal R}$.

2: 设置块大小 $B_{c}=\left\lceil\frac{M}{4d}\right\rceil,B_{r}=\min\left(\left\lceil\frac{M}{4d}\right\rceil,d\right)$.

3: 将 $\mathbf{Q}$ 分成 $T_{r}=\left\lceil\frac{N}{B_{r}}\right\rceil$ 块, 每块 $\mathbf{Q}_{1},\dots,\mathbf{Q}_{T_{r}}$, 大小为 $B_{r}\times d$, 并将 $\mathbf{K},\mathbf{V}$ 分成 $T_{c}=\left\lceil\frac{N}{B_{c}}\right\rceil$ 块 $\mathbf{K}_{1},\dots,\mathbf{K}_{T_{c}}$ 和 $\mathbf{V}_{1},\dots,\mathbf{V}_{T_{c}}$, 每块大小为 $B_{c}\times d$.

4: 将 $\mathbf{O}$ 分成 $T_{r}$ 块, 每块大小为 $B_{r}\times d$, 将 $\mathbf{\mathrm{dO}}$ 分成 $T_{r}$ 块, 每块大小为 $B_{r}\times d$, 将 $\ell$ 分成 $T_{r}$ 块, 每块大小为 $B_{r}$, 将 $m$ 分成 $T_{r}$ 块, 每块大小为 $B_{r}$.

5: 在 HBM 中初始化 $\mathbf{\mathrm{dQ}}=(0)_{N\times d}$ 并将其划分为 $T_{r}$ 块, 每块大小为 $B_{r}\times d$. 在 HBM 中初始化 $\mathbf{\mathrm{dK}}=(0)_{N\times d},\mathbf{\mathrm{dV}}=(0)_{N\times d}$ 并将 $\mathbf{\mathrm{dK}},\mathbf{\mathrm{dV}}$ 划分为 $T_{c}$ 块 $\mathbf{\mathrm{dK}}_{1},\dots,\mathbf{\mathrm{dK}}_{T_{c}}$ 和 $\mathbf{\mathrm{dV}}_{1},\dots,\mathbf{\mathrm{dV}}_{T_{c}}$, 每块大小为 $B_{c}\times d$.

6: 对于 $1\leq j\leq T_{c}$ 执行

7: 将 $\mathbf{K}_{j},\mathbf{V}_{j}$ 从 HBM 加载到片上 SRAM.

8: 在 SRAM 上初始化 $\tilde{\mathbf{\mathrm{dK}}}_{j}=(0)_{B_{c}\times d},\tilde{\mathbf{\mathrm{dV}}}_{j}=(0)_{B_{c}\times d}$.

9: 对于 $1\leq i\leq T_{r}$ 执行

10: 将 $\mathbf{Q}_{i},\mathbf{O}_{i},\mathbf{\mathrm{dO}}_{i},\mathbf{\mathrm{dQ}}_{i},\ell_{i},m_{i}$ 从 HBM 加载到片上 SRAM.

11: 在芯片上计算 $\mathbf{S}_{\mathrm{ij}}=\tau\mathbf{Q}_{i}\mathbf{K}_{j}^\top\in\mathbb{R}^{B_{r}\times B_{c}}$.

12: 在芯片上计算 $\mathbf{S}_{\mathrm{ij}}^{\mathrm{masked}}=\mathrm{mask}(\mathbf{S}_{\mathrm{ij}})$.

13: 在芯片上计算 $\mathbf{P}_{\mathrm{ij}}=\mathrm{diag}(l_{i})^{-1}\exp(\mathbf{S}_{\mathrm{ij}}^{\mathrm{masked}}-m_{i})\in\mathbb{R}^{B_{r}\times B_{c}}$.

14: 在芯片上, 计算丢弃掩码 $\mathbf{Z}_{\mathrm{ij}}\in\mathbb{R}^{B_{r}\times B_{c}}$, 其中每个条目的值为 $\frac{1}{1-p_{\mathrm{drop}}}$ 的概率为 $1-p_{\mathrm{drop}}$, 值为 0 的概率为 $p_{\mathrm{drop}}$.

15: 在芯片上, 计算 $\mathbf{P}_{\mathrm{ij}}^{\mathrm{dropped}}=\mathbf{P}_{\mathrm{ij}}\circ\mathbf{Z}_{\mathrm{ij}}$ (逐点相乘).

16: 在芯片上, 计算 $\tilde{\mathbf{\mathrm{dV}}_{j}}\leftarrow\tilde{\mathbf{\mathrm{dV}}_{j}}+(\mathbf{P}_{\mathrm{ij}}^{\mathrm{dropped}})^{\top}\mathbf{\mathrm{dO}}_{i}\in\mathbb{R}^{B_{c}\times d}$.

17: 在芯片上, 计算 $\mathbf{\mathrm{dP}}_{\mathrm{ij}}^{\mathrm{dropped}}=\mathbf{\mathrm{dO}}_{i}\mathbf{V}_{j}^{\top}\in\mathbb{R}^{B_{r}\times B_{c}}$.

18: 在芯片上, 计算 $\mathbf{\mathrm{dP}}_{\mathrm{ij}}=\mathbf{\mathrm{dP}}_{\mathrm{ij}}^{\mathrm{dropped}}\circ\mathbf{Z}_{\mathrm{ij}}$ (逐点相乘).

19: 在芯片上, 计算 $D_{i}=\mathrm{rowsum}(\mathbf{\mathrm{dO}}_{i}\circ\mathbf{O}_{i})\in\mathbb{R}^{B_{r}}$.

20: 在芯片上, 计算 $\mathbf{\mathrm{dS}}_{\mathrm{ij}}=\mathbf{P}_{\mathrm{ij}}\circ(\mathbf{\mathrm{dP}}_{\mathrm{ij}}-D_{i})\in\mathbb{R}^{B_{r}\times B_{c}}$.

21: 将 $\mathbf{\mathrm{dQ}}_{i}\leftarrow\mathbf{\mathrm{dQ}}_{i}+\tau\mathbf{\mathrm{dS}}_{\mathrm{ij}}\mathbf{K}_{j}\in\mathbb{R}^{B_{r}\times d}$ 写入 HBM.

22: 在芯片上, 计算 $\tilde{\mathbf{\mathrm{dK}}}_{j}\leftarrow\tilde{\mathbf{\mathrm{dK}}}_{j}+\tau\mathbf{\mathrm{dS}}_{\mathrm{ij}}^{\top}\mathbf{Q}_{i}\in\mathbb{R}^{B_{c}\times d}$.

23: 结束 for

24: 将 $\mathbf{\mathrm{dK}}_{j}\leftarrow\tilde{\mathbf{\mathrm{dK}}_{j}},\mathbf{\mathrm{dV}}_{j}\leftarrow\tilde{\mathbf{\mathrm{dV}}_{j}}$ 写入 HBM.

25: 循环结束

26: 返回 $\mathbf{\mathrm{dQ}},\mathbf{\mathrm{dK}},\mathbf{\mathrm{dV}}$.
...
两种方法之间的第二个区别是从每个块总结信息以传递到下一个块的方式. [Staats21] 使用其临时输出以及 softmax 归一化统计来总结每个块. 在前向传播结束时, 所有块的临时输出结合这些统计生成最终输出. FlashAttention 则在处理每个块后增量更新输出 ([算法 1](#alg1) 第 [12] 行(#alg1.l12 "在算法 1 ‣ 3.1 块和重计算下的高效注意力算法 ‣ 3 FlashAttention: 算法, 分析与扩展 ‣ 具有 IO 感知的快速且节省内存的精确注意力")), 因此只需要一份输出副本 (而不是 $K$ 份对应 $K$ 个块). 这意味着 FlashAttention 的总内存需求比 [Staats21] 小.
...
主导的浮点运算 (FLOPs) 来自矩阵乘法. 在内循环中, ([算法 1](#alg1) 第 [9] 行(#alg1.l9 "在算法 1 ‣ 3.1 有瓦片化和重计算的高效注意力算法 ‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 具有 IO 感知的快速且内存高效的精确注意力")), 我们计算 $\mathbf{Q}_{i}\mathbf{K}_{j}^{\top}\in\mathbb{R}^{B_{r}\times B_{c}}$ 用于 $\mathbf{Q}_{i}\in\mathbb{R}^{B_{r}\times d}$ 和 $\mathbf{K}_{j}\in\mathbb{R}^{B_{c}\times d}$, 这需要 $O(B_{r}B_{c}d)$ FLOPs. 我们还计算 ([算法 1](#alg1) 第 [12] 行(#alg1.l12 "在算法 1 ‣ 3.1 有瓦片化和重计算的高效注意力算法 ‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 具有 IO 感知的快速且内存高效的精确注意力")) $\tilde{\mathbf{P}}_{\mathrm{ij}}\mathbf{V}_{j}\in\mathbb{R}^{B_{r}\times d}$ 用于 $\tilde{\mathbf{P}}_{\mathrm{ij}}\in\mathbb{R}^{B_{r}\times B_{c}}$ 和 $\mathbf{V}_{j}\in\mathbb{R}^{B_{c}\times d}$, 这需要 $O(B_{r}B_{c}d)$ FLOPs. 我们执行内循环 $T_{c}T_{r}=\left\lceil\frac{N}{B_{c}}\right\rceil\left\lceil\frac{N}{B_{r}}\right\rceil$ 次. 因此 FLOPs 总数为
...
我们现在通过对 $j$ 关于 $0\leq j\leq T_{c}$ 进行归纳来证明算法的正确性. 设 $\mathbf{K}_{:j}\in\mathbb{R}^{\mathrm{jB}_{c}\times d}$ 为 $\mathbf{K}$ 的前 $\mathrm{jB}_{c}$ 行, 类似地, $\mathbf{V}_{:j}\in\mathbb{R}^{\mathrm{jB}_{c}\times d}$ 为 $\mathbf{V}$ 的前 $\mathrm{jB}_{c}$ 行. 设 $\mathbf{S}_{:,:j}=\mathbf{Q}\mathbf{K}_{:j}^{\top}\in\mathbb{R}^{N\times \mathrm{jB}_{c}}$, 以及 $\mathbf{P}_{:,:j}=\mathrm{softmax}(\mathbf{S}_{:,:j})\in\mathbb{R}^{N\times \mathrm{jB}_{c}}$ (按行应用 softmax). 设 $m^{j},\ell^{(j)},\mathbf{O}^{(j)}$ 为在外层循环第 $j$ 次迭代后, HBM 中 $m,\ell,\mathbf{O}$ 的值 ([算法 1](#alg1), 第 [5] 行 (#alg1.l5 "在算法 1 ‣ 3.1 具有平铺和重新计算的高效注意力算法 ‣ 3 FlashAttention: 算法, 分析与扩展 ‣ FlashAttention: 快速且具有 IO 感知的内存高效精确注意力")). (注意这些 $m,\ell,\mathbf{O}$ 的值在外层循环的每次迭代后都会被更新. ) 我们想要证明, 在外层循环第 $j$ 次迭代后, 我们已经在 HBM 中计算了:
...
根据我们的初始化 ([算法 1](#alg1) 第 [2](#alg1.l2) 行), 这个结论对于 $j=0$ (即在外循环的任何迭代执行之前) 是成立的. 假设该结论对某个 $j=0,\dots,T_{c}-1$ 成立. 我们想要证明该结论也对 $j+1$ 成立. 确实, 当我们在外循环的 $(j+1)$ 次迭代中在内循环中更新统计数据 ([算法 1](#alg1) 第 [10](#alg1.l10) 行) 时, 我们更新 $m^{(j+1)}=\max(m^{(j)},\tilde{m})$, 其中 $\tilde{m}\in\mathbb{R}^{N}$ 是 $\mathbf{S}_{:,j:j+1}$ 的行最大值, $\mathbf{S}_{:,j:j+1}$ 是从 $\mathbf{S}$ 的第 $\mathrm{jB}_{c}$ 列到第 $(j+1)B_{c}-1$ 列的切片. 这意味着
...
在计算矩阵乘法 $\mathbf{S}=\mathbf{Q}\mathbf{K}^{\top}$ 的第一步中, 输入 $\mathbf{Q},\mathbf{K}$ 从 HBM 中读取, 输出 $\mathbf{S}\in\mathbb{R}^{N\times N}$ 写入 HBM ([算法](#alg0) 第 [1] 行 (#alg0.l1 "在算法 0 ‣ 2.2 标准注意力实现 ‣ 2 背景 ‣ FlashAttention: 具有 IO 感知的快速且节省内存的精确注意力")). 这会产生 $\Theta(\mathrm{Nd}+N^{2})$ 次 HBM 访问.

在计算 $\mathbf{P}=\mathrm{softmax}(\mathbf{S})$ 的第二步中, 输入 $\mathbf{S}$ 从 HBM 读取, 输出 $\mathbf{P}$ 写入 HBM (［算法］(#alg0 "在 2.2 标准注意力实现 ‣ 2 背景 ‣ FlashAttention: 快速且内存高效的精确注意力与 IO 感知") 第 [2] 行](#alg0.l2 "在算法 0 ‣ 2.2 标准注意力实现 ‣ 2 背景 ‣ FlashAttention: 快速且内存高效的精确注意力与 IO 感知")). 这将产生 $\Theta(N^{2})$ 次 HBM 访问.

在计算 $\mathbf{O}=\mathbf{P}\mathbf{V}$ 的最后一步中, 输入 $\mathbf{P},\mathbf{V}$ 从全局内存读取, 输出 $\mathbf{O}$ 写入 HBM ([算法](#alg0) 第 [3] 行 (#alg0.l3 "在算法 0 ‣ 2.2 标准注意力实现 ‣ 2 背景 ‣ FlashAttention: 具有 IO 感知的快速且节省内存的精确注意力")). 这会产生 $\Theta(\mathrm{Nd}+N^{2})$ 次 HBM 访问.

总体来看, 标准注意力实现需要 $\Theta(\mathrm{Nd}+N^{2})$ 次全局内存访问.

我们现在分析流式注意力的 IO 复杂度.

根据 [算法 1](#alg1), 我们看到 $\mathbf{K}$ 和 $\mathbf{V}$ 的每个元素只需从 HBM 加载一次 ([算法 1](#alg1) 第 [6](#alg1.l6) 行). 我们对 $\mathbf{Q}$ 和 $\mathbf{O}$ 进行 $T_{c}$ 次遍历, 每次遍历将所有 $\mathbf{Q}$ 和所有 $\mathbf{O}$ 加载到 HBM ([算法 1](#alg1) 第 [8](#alg1.l8) 行). 因此 HBM 访问的次数为 $\Theta\left(\mathrm{Nd}+\mathrm{NdT}_{c}\right)=\Theta(\mathrm{NdT}_{c})$.
...
0: 矩阵 $\mathbf{Q},\mathbf{K},\mathbf{V}\in\mathbb{R}^{N\times d}$ 位于 HBM 中, 片上 SRAM 大小为 $M$, softmax 缩放常数为 $\tau\in\mathbb{R}$, 掩码函数为 mask, dropout 概率为 $p_{\mathrm{drop}}$, 块大小为 $B_{c}=\left\lceil\frac{M}{4d}\right\rceil,B_{r}=\min\left(\left\lceil\frac{M}{4d}\right\rceil,d\right)$, 块稀疏掩码为 $M\in\{0,1\}^{N/B_{r}\times N/B_{c}}$. #alg5 1: 初始化伪随机数生成器状态 ${\cal R}$ 并保存到 HBM. ZXQ00013QXZ 2: 初始化 HBM 中的 $\mathbf{O}=(0)_{N\times d}\in\mathbb{R}^{N\times d},\ell=(0)_{N}\in\mathbb{R}^{N},m=(-\infty)_{N}\in\mathbb{R}^{N}$.
...
12: 在芯片上, 计算 $\tilde{m}_{\mathrm{ij}}=\mathrm{rowmax}(\mathbf{S}_{\mathrm{ij}}^{\mathrm{masked}})\in\mathbb{R}^{B_{r}}$, $\tilde{\mathbf{P}}_{\mathrm{ij}}=\exp(\mathbf{S}_{\mathrm{ij}}^{\mathrm{masked}}-\tilde{m}_{\mathrm{ij}})\in\mathbb{R}^{B_{r}\times B_{c}}$ (逐点) 和 $\tilde{\ell}_{\mathrm{ij}}=\mathrm{rowsum}(\tilde{\mathbf{P}}_{\mathrm{ij}})\in\mathbb{R}^{B_{r}}$.
...
我们遵循 Long-range arena 论文 [Tay20], Long-range arena 仓库([https://github.com/google-research/long-range-arena](https://github.com/google-research/long-range-arena))以及 Nyströmformer 复现 [Xiong21] 中的超参数. 为了给基线方法宽松, 如果我们无法在任意五个任务中的任何一个再现某个基线的性能, 我们将报告 [Tay20] 或 [Xiong21] 中该基线在该任务上的更好性能.
...
我们将我们的方法/实现与 Apex FMHA ([https://github.com/NVIDIA/apex/tree/master/apex/contrib/csrc/fmha](https://github.com/NVIDIA/apex/tree/master/apex/contrib/csrc/fmha]）进行比较。
...
|块状稀疏|0.96 ZX0028QXZ MoE-65536-h|1.04|1.66|2.16|5.41|16.15|-|-|-|-|
|长格式|0.99|0.98|0.99|1.56|4.79|11.07|32.98|-|-|-|
|大鸟|0.96 ZX0028QXZ MoE-65536-h|1.02|1.02|1.48|5.05|11.59|34.16|-|-|-|
...
|LSformer|1.66|1.61|3.09|7.42
ZX0014QXZ 19.68
ZX0015QXZ 38.35
ZX0016QXZ 74.92
ZX0017QXZ 147.86| 19.68| 38.35| 74.92| 147.86|-|-|
...
|改革者|1.83 ZX0022QXZ 3.50|2.96|5.31|10.33|21.19|43.42|91.96|201.34|-|-|
...
|块稀疏闪烁注意力|0.91|0.83|0.94|0.92|1.83 ZX0022QXZ 3.50| 3.50|7.02|13.56|26.71|53.92|
...
|块稀疏闪烁注意力|0.56|0.52|0.63|0.65|0.61|0.96 ZX0028QXZ MoE-65536-h|1.69|3.02|5.69|11.77|
...
|长格式|1.27|1.23|1.24|1.85|4.99|10.21|24.89|-|-|-|
|大鸟|1.43|1.50|1.44|1.69|5.25|10.86|26.26|-|-|-|
|闪光关注|0.11|0.16 ZX0030QXZ 0.38|0.52|1.62|5.45|21.57|84.75|336.00|1338.56|5343.19|
|块稀疏闪烁注意力|0.11|0.12|0.16 ZX0030QXZ 0.38| 0.38|1.20|2.34|4.69|9.10|18.74|37.04|
...
[+1]: FlashAttention 代码可在 [https://github.com/HazyResearch/flash-attention](https://github.com/HazyResearch/flash-attention] 获得.
