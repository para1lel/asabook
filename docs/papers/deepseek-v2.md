...
过去几年里, 大语言模型 (LLM) [Ope22, Ope23, Ant23, Goo23] 发展迅速, 让我们得以一窥通用人工智能 (AGI) 的曙光. 总体而言, LLM 的智能往往随参数数量增加而提升, 从而在各项任务上表现出涌现能力 [Wei22d]. 然而, 这种提升需要更多训练算力, 也可能降低推理吞吐量. 这些约束给 LLM 的广泛采用和使用带来了显著挑战. 为解决这一问题, 我们提出 DeepSeek-V2, 一个性能强劲的开源混合专家 (MoE) 语言模型, 通过新的 Transformer 架构实现经济训练与高效推理. 它共有 236B 参数, 每个 token 激活其中的 21B, 并支持 128K token 的上下文长度.
...
2. 对于前馈网络 (FFN), 我们沿用 DeepSeekMoE 架构 [Dai24], 该架构采用细粒度专家切分与共享专家隔离, 让专家有更大的专业化潜力. 与 GShard [Lep20] 等传统 MoE 架构相比, DeepSeekMoE 的优势明显, 使我们能够以经济成本训练出强大的模型. 由于训练中采用专家并行, 我们还设计了控制通信开销和维持负载均衡的补充机制. 结合这两项技术后, DeepSeek-V2 同时具备强大的性能 ([图 1](#figure-01)(a)), 经济的训练成本和高效的推理吞吐量 ([图 1](#figure-01)(b)).
...
我们在英语和中文的多项基准上评测 DeepSeek-V2, 并将其与代表性开源模型比较. 评测结果表明, 即使仅有 21B 激活参数, DeepSeek-V2 依然在开源模型中取得顶尖性能, 成为最强的开源 MoE 语言模型. [图 1](#figure-01) 表明, 在 MMLU 上, DeepSeek-V2 仅凭少量激活参数便取得顶级表现. 此外, 如 [图 1](#figure-01) 所示, 与 DeepSeek 67B 相比, DeepSeek-V2 节省了 42.5% 的训练成本, 将 KV cache 减少了 93.3%, 并将最大生成吞吐量提升至 5.76 倍. 我们还在开放式基准上评测了 DeepSeek-V2 Chat (SFT) 与 DeepSeek-V2 Chat (RL). 其中, DeepSeek-V2 Chat (RL) 在 AlpacaEval 2.0 [Dub24a] 上取得了 38.9 的长度控制胜率, 在 MT-Bench [Sto23e] 上取得 8.97 的总分, 在 AlignBench [Liu23m] 上取得 7.91 的总分. 英语开放式对话评测表明, DeepSeek-V2 Chat (RL) 在开源对话模型中具有顶尖性能. AlignBench 的结果还表明, 在中文上, DeepSeek-V2 Chat (RL) 超越了所有开源模型, 甚至击败了大多数闭源模型.
...
对于 DeepSeek-V2, 除了对路由专家进行朴素的 top-K 选择外, 我们还让每个 token 的目标专家分布在至多 $M$ 个设备上. 具体而言, 对于每个 token, 我们先选择亲和度分数最高的专家所在的 $M$ 个设备. 然后, 我们在这些设备的专家之间进行 top-K 选择. 实际中我们发现, 当 $M \geq 3$ 时, 设备受限路由的性能与不受限 top-K 路由大致相当.
...
其中 $\alpha_{3}$ 是名为通信均衡因子的超参数. 设备受限路由机制确保每个设备向其他设备传输至多 $M T$ 个隐藏状态. 同时, 通信均衡损失促使每个设备从其他设备接收约 $M T$ 个隐藏状态. 它让设备间的信息交换保持均衡, 从而提高通信效率.
...
所有实验均在配备 NVIDIA H800 GPU 的集群上进行. 集群中的每个节点包含 8 个 GPU, 节点内通过 NVLink 和 NVSwitch 连接. 节点间则通过 InfiniBand 互连通信.
...
在我们此前研究 [Dee24e] 的基础上, 我们整理了 150 万个指令调优实例, 其中 120 万个用于有用性训练, 30 万个用于安全性训练. 与初始版本相比, 我们提高了数据质量, 以减少幻觉回答并改善写作能力. 我们对 DeepSeek-V2 微调 2 个 epoch, 学习率设为 $5 \times 10^{-6}$. 评测 DeepSeek-V2 Chat (SFT) 时, 除少数代表性多项选择任务 (MMLU 和 ARC) 外, 我们主要采用生成式基准. 我们还对 DeepSeek-V2 Chat (SFT) 进行了指令遵循评测 (IFEval) [Zho23a], 指标为提示级宽松准确率. 此外, 我们使用 2023 年 9 月 1 日至 2024 年 4 月 1 日期间的 LiveCodeBench [Jai25a] 题目评测对话模型. 除标准基准外, 我们还在 MT-Bench [Sto23e], AlpacaEval 2.0 [Dub24a] 和 AlignBench [Liu23m] 等开放式对话基准上评测模型. 作为比较, 我们也在相同的内部评测框架和设置下评测 Qwen1.5 72B Chat, LLaMA-3-70B Instruct 和 Mistral-8x22B Instruct. DeepSeek 67B Chat 则直接采用此前发布中报告的评测结果.
...
**训练策略.** 初步实验表明, 推理数据 (例如代码和数学提示) 上的 RL 训练具有不同于通用数据训练的特征. 例如, 模型的数学和编码能力可以在较长的训练过程中持续提升. 因此, 我们采用两阶段 RL 训练策略: 先进行推理对齐, 再进行人类偏好对齐. 第一阶段中, 我们为代码和数学推理任务训练奖励模型 $\mathit{RM}_{\mathrm{reasoning}}$, 并根据 $\mathit{RM}_{\mathrm{reasoning}}$ 的反馈优化策略模型:
...
在第二阶段的人类偏好对齐中, 我们采用多奖励框架, 从有用性奖励模型 $\mathit{RM}_{\mathrm{helpful}}$, 安全性奖励模型 $\mathit{RM}_{\mathrm{safety}}$ 和基于规则的奖励模型 $\mathit{RM}_{\mathrm{rule}}$ 获取奖励. 回复 $o_i$ 的最终奖励为
...
为了获得 RL 训练所需的可靠奖励模型, 我们谨慎收集偏好数据, 并严格进行质量过滤和比例调整. 代码偏好数据来自编译器反馈, 数学偏好数据来自真实标签. 训练奖励模型时, 我们用 DeepSeek-V2 Chat (SFT) 初始化模型, 再使用逐点损失或成对损失训练. 实验中, RL 训练可以充分挖掘并激活模型潜力, 使其从候选回复中选出正确且令人满意的答案.

**针对训练效率的优化.** 在超大模型上进行 RL 训练对训练框架提出了很高要求. 要控制 GPU 内存和 RAM 压力, 同时保持较快的训练速度, 需要细致的工程优化. 为此, 我们实施了以下优化.

1. 首先, 我们提出一种混合引擎, 分别为训练和推理采用不同的并行策略, 以提高 GPU 利用率.
2. 其次, 我们使用大批量的 vLLM [Kwo23] 作为推理后端, 加快推理速度.
3. 第三, 我们仔细设计了一种将模型卸载到 CPU 并重新载入 GPU 的调度策略, 在训练速度与内存消耗之间取得接近最优的平衡.
...
**开放式生成的评测.** 我们继续在开放式对话基准上评测模型. 英语开放式对话生成采用 MT-Bench 和 AlpacaEval 2.0. [表 4](#table-04) 的结果表明, DeepSeek-V2 Chat (RL) 的性能显著优于 DeepSeek-V2 Chat (SFT), 说明 RL 训练改善了对齐效果. 与其他开源模型相比, DeepSeek-V2 Chat (RL) 在两个基准上都优于 Mistral 8x22B Instruct 和 Qwen1.5 72B Chat. 与 LLaMA3 70B Instruct 相比, DeepSeek-V2 Chat (RL) 在 MT-Bench 上具有竞争力, 在 AlpacaEval 2.0 上则显著更好. 这些结果表明, DeepSeek-V2 Chat (RL) 能够生成高质量且符合上下文的回复, 在指令式对话任务上尤其如此.
...
**SFT 数据量.** 是否需要大规模 SFT 语料一直存在激烈争论. 先前工作 [You24a, Zho24a] 认为, 少于 1 万条 SFT 数据就足以得到令人满意的结果. 然而, 我们的实验中, 数据少于 1 万条时, IFEval 性能会显著下降. 一种可能的解释是, 语言模型需要一定量的数据才能形成特定技能. 所需数据量可能随模型规模增大而减少, 但无法降为零. 这一观察说明, 要让 LLM 具备所需能力, 仍需足量数据. SFT 数据的质量同样重要, 尤其是对写作或开放式问题等任务而言.
...
DeepSeek-V2 及其对话版本也有其他 LLM 常见的局限, 包括预训练后知识不再持续更新, 可能生成不实信息 (例如未经证实的建议), 以及可能出现幻觉. 此外, 由于训练数据主要由中文和英文内容组成, 模型对其他语言的掌握可能有限. 在中文和英语以外的场景中, 应谨慎使用.

DeepSeek 将坚持长期主义, 持续投入开源大模型, 逐步逼近人工通用智能这一目标.
...
[图 5](#figure-05) 进一步给出了 HumanEval 与 LiveCodeBench 的结果, 其中 LiveCodeBench 的题目选自 2023 年 9 月 1 日至 2024 年 4 月 1 日. 如图所示, DeepSeek-V2 Chat (RL) 在 LiveCodeBench 上的能力很强, Pass@1 分数甚至超过了一些巨型模型. 这一表现说明, DeepSeek-V2 Chat (RL) 处理实时编码任务的能力很强.
