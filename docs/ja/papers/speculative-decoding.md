...
接頭辞 $x_{<t}$ が与えられたとき、 *acceptance rate $\beta_{x_{<t}}$* とは [2.3 節](#section-02-03) に従う speculative sampling により $x_{t}\sim q(x_{t}|x_{<t})$ を受理する確率である [+2]。
...
$D_{\mathrm{LK}}(p,q)=\sum_{x}|p(x)-M(x)|=\sum_{x}|q(x)-M(x)|$、ただし $M(x)=\frac{p(x)+q(x)}{2}$。
...
$$
\begin{aligned}
D_{\mathrm{LK}}(p,q)&\text{ は }[0,1]\text{ 上の対称な発散である。}\\
D_{\mathrm{LK}}(p,q)=0&\iff p=q\text{。}\\
D_{\mathrm{LK}}(p,q)=1&\iff p\text{ と }q\text{ の台は互いに素である。}
\end{aligned}
$$
...
*cost coefficient* $c$ を、 $M_{q}$ の 1 回の実行時間と $M_{p}$ の 1 回の実行時間の比とする。
...
[アルゴリズム 1](#algorithm-01) による総 walltime の期待改善率は $\frac{1-\alpha^{\gamma+1}}{(1-\alpha)({\gamma}c+1)}$ である。
...
$\alpha>c$ なら、改善を得る $\gamma$ が存在し、改善率は少なくとも $\frac{1+\alpha}{1+c}$ である。
...
$\hat{c}$ を、近似モデル $M_{q}$ の 1 トークンあたりの算術演算数とターゲットモデル $M_{p}$ のそれとの比とする。

###### 定理 3.11。

[アルゴリズム 1](#algorithm-01) の総演算回数の期待増加係数は $\frac{(1-\alpha)({\gamma}\hat{c}+\gamma+1)}{1-\alpha^{\gamma+1}}$ である。
