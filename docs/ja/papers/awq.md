...
トランスフォーマーに基づく大規模言語モデル（LLM）は [Vas17]、さまざまなベンチマークで優れた性能を示しています [Bro77, Zha22a, Tou23, Les23]。しかし、大規模モデルのサイズは高いサービングコストにつながります。例えば、GPT-3 は 175B パラメータを持ち、FP16 では 350GB ですが、最新の H100 GPU は 96GB のメモリしか持たず、エッジデバイスはなおさらです。
