...
トランスフォーマーベースの言語モデル [Xivar17, PreTra18, Blog19, Xivbb18, CoRR07, CoRR06, Xive10] は、計算リソースの大規模化が可能になり、データセットが大きくなるにつれて、近年急速な進展を遂げてきました。最近の研究 [Xivl05, Xivm09] は、大規模言語モデルがゼロショットまたは少数ショット学習者として効果的であり、多くの NLP タスクやデータセットで高精度を示すことを明らかにしています。これらの大規模言語モデルには、クライアントフィードバックの要約、自動対話生成、セマンティック検索、コード自動補完など、魅力的な下流アプリケーションが多数存在します [GPT00, Code21, Copilo00]。その結果、最先端 NLP モデルのパラメータ数は指数関数的に増加しています（[図1](#figure-01)）。しかし、このようなモデルのトレーニングは以下の2つの理由から困難です：（a） このモデルのパラメータを最大級の GPU のメインメモリ内に収めることはもはや不可能である（NVIDIA は最近 80GB-A100 カードをリリース）、（b） もし単一 GPU にモデルを収めることができても（例えば、ホストメモリとデバイスメモリ間でパラメータをスワップすることで [Xivaf21]）、必要な計算量の多さにより、現実的でない長時間のトレーニング時間となる可能性がある（例えば 1750 億パラメータの GPT-3 のトレーニング [Xivl05] には、単一 V100 NVIDIA GPU で約 288 年を要する）。これにより並列処理が必要となります。データ並列スケールアウトは通常うまく機能しますが、以下の2つの制約があります：a） 一定のポイントを超えると、GPU ごとのバッチサイズが小さくなり、GPU の使用率が低下し通信コストが増加する、b） 使用できるデバイスの最大数はバッチサイズに制限され、トレーニングに使用できるアクセラレータの数が制限される。
...
パイプライン並列処理に使用されるスケジュールは、通信量、パイプラインバブルのサイズ、そしてアクティベーションを保存するために使用されるメモリに影響を与えます。我々は、以前に提案されたスケジュール[Systeh19, PMLR21]と比較して、同等のメモリフットプリントでスループットを最大10%改善できる新しいインターリーブスケジュールを提案します。
...
Parallelism戦略（例えばFlexFlow [MLSysb18]、PipeDream [Princb19]、Tarnawskiら [Systec20]、およびDAPPLE [Progra21]）の探索空間を自動的に探索するわけではありませんが、代わりに実際にうまく機能するヒューリスティックを提示しています（§[3](#S3)を参照）。
...
パイプライン並列化では、モデルの層を複数のデバイスに分割します。同じトランスフォーマーブロックが繰り返されるモデルに使用する場合、各デバイスに均等な数のトランスフォーマー層を割り当てることができます。層の割り当てがより難しい非対称なモデルアーキテクチャについては考慮せず、この問題の解決は関連研究[Princb19, MLSysb18, Systec20]に委ねます。
...
厳密なオプティマイザの意味論を*完全に*保持するために、定期的なパイプラインフラッシュを導入し、オプティマイザのステップがデバイス間で同期されるようにします。各バッチの開始時と終了時にはデバイスはアイドル状態になります。このアイドル時間を*パイプラインバブル*と呼び、できるだけ小さくしたいと考えています。PipeMare、PipeDream、PipeDream-2BW などの非同期および有界遅延アプローチ [Systea21, System21, Princb19, PMLR21] はフラッシュを完全に廃止しますが、重み更新の意味論を緩めます。そのような方式の検討は将来の課題とします。
...
テンサーモデル並列化では、モデルの個々の層が複数のデバイスに分割されます。本論文では、Megatron [Xivm09]がトランスフォーマー層、すなわち言語モデルの基盤として使用する特定の分割戦略を使用します。CNNなど他のタイプのモデルにも同様の考え方を適用できます。この戦略を簡単に概説し、[図 5](#figure-05)に示します。
...
**図5.** テンソルモデル並列で分割されたトランスフォーマーモデルのブロック（図はMegatron [Xivm09]から借用）。$f$と$g$は共役です。$f$は順伝播では恒等演算子、逆伝播ではオールリデュースであり、$g$はその逆です。
...
活性化再計算 [Systeh19, Xivaq16, TOMSb00, Systeb20] はオプションの技術で、順伝播を逆伝播の直前にもう一度実行することによって追加のメモリフットプリントのために実行される計算操作の数を増やすというトレードオフを行います（パイプライン段階ごとの入力活性化のみを保存し、はるかに多い中間活性化の全セットを保存するのではありません）。活性化再計算は、メモリフットプリントを許容できる低さに保つために、パイプライン並列性を用いた比較的大きなモデルをトレーニングする際に必要です。PipeDream-2BW [PMLR21] のような以前の研究では、活性化再計算のパフォーマンスへの影響が検討されています。
...
活性化メモリ消費をさらに削減するために、活性化分割 （activation partitioning [Xivf10]） のような他の手法もテンソルモデル並列処理と併用することができます。

## 4. 実装

我々は PTD-P を Megatron-LM コードベースの拡張として実装しました。我々の実装は PyTorch [Systei19] を使用して構築されています。デバイス間の通信には NCCL [NCCL00] を使用します。良好な性能を得るために、通信と計算の両方を対象とした最適化を実装しました。以下にその概要を示します。
...
パイプラインモデル並列処理は、大規模モデルの訓練に使用される一般的な技術です。パイプライン並列処理にはいくつかの種類があります。本稿で議論されているモードは、*厳密な*オプティマイザの意味論を保証するためにフラッシュを使用します。TeraPipe [Xivad21] は、GPTのような自己回帰モデルの単一訓練シーケンス内のトークン間で細粒度のパイプライン並列処理を提供します。PipeTransformer [Xivac21] は、“安定した”重みを持つ層を凍結し、代わりに残りの“アクティブ”な層の訓練にリソースを割り当てることで、パイプライン化とデータ並列度を柔軟に調整します。HetPipe [Parall20] は、異種アクセラレータのセット上でパイプラインとデータ並列処理を組み合わせて使用します。パイプライン並列処理は、緩やかな意味論でも実装可能です：PipeDream-2BW [PMLR21] は二つの重みバージョンを保持し、高価なフラッシュなしで1ステールの重み更新を保証します。一方、PipeMare [Systea21] と Kosson ら [System21] は非同期パイプライン並列処理を使用します。これらの技術は、本稿で検討されたパイプラインフラッシュを伴う技術に比べてスループットを改善しましたが、収束速度や最終的な精度の点で犠牲になる可能性があります。さらに、パイプライン並列処理は単独では、モデルの層の数に等しい数のデバイスにしかスケールできないため、特定のモデルアーキテクチャにとって制限となります。

PipeDream [Princb19] は、デバイス間通信を減らすために、体系的な方法でパイプライン並列性とデータ並列性を組み合わせました。DeepSpeed [Everyo00] は、パイプライン並列性とテンソルおよびデータ並列性を組み合わせて最大1兆パラメータのモデルを訓練しましたが、この論文で示されたスループットよりも低くなりました（ピークの52%対36%）その理由はいくつかあります：演算子グラフの大部分を計算バウンドに保つための演算子融合、パイプラインバブルサイズを最小化するより効率的なパイプライン並列スケジュール、高速ハードウェア（A100対V100 GPUおよび同一サーバーおよび異なるサーバー間のGPU間の高帯域幅リンク）、およびより多くのGPUへのスケーリングです。我々は、この高いスループットが推定訓練時間をはるかに実用的にすること（約3か月）を強調したいと思います。同等のサイズのモデルを訓練するためには、総スループット37.6 petaFLOP/s では約40か月かかります。より大きなモデルへのスケーリングも可能ですが、訓練時間を実用的に保つためにはより多くのGPUが必要です。
