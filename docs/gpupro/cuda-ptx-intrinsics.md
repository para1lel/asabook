...
**Election.** `T.ptx.elect_sync()` 从一个 warp 的 active lane 中选择
...
`__syncthreads()`, 要求 CTA 的所有 thread 到达. warpgroup 进入不同
角色分支后, 不能把 `cta_sync()` 放进其中一个 warpgroup 的分支, 否则其他
warpgroup 无法到达, kernel 会 deadlock. 硬件提供 16 个 named barrier
(ID 0 到 15); `T.cuda.warpgroup_sync(10)` 只同步一个 warpgroup 的
thread. 不同 warpgroup 使用不同 ID, 例如
