...
::: info 概览
- GPU kernel 的执行由线程层级组织: thread, warp, warpgroup, CTA, cluster 和 grid 分别对应不同的协作粒度. Blackwell 上的许多操作都有自己的 scope, 例如 TMA copy 由单个 thread 发起, 完整的 TMEM accumulator 由四个 warp 分别读取各自的 32-lane window, 2-CTA cooperative MMA 则跨越两个 CTA.
- 数据会分布在多个位置. GMEM, SMEM, TMEM 和寄存器在容量、延迟和访问范围上各有取舍; cluster 还通过 DSMEM 让一个 CTA 访问另一个 CTA 的 shared memory. 高性能 kernel 需要安排数据在这些空间之间高效移动.
- 计算和数据搬运由不同硬件引擎完成. CUDA Core 处理地址计算、控制流和标量逻辑, Tensor Core 执行主要矩阵计算, TMA 负责异步搬运数据. 最后用一条 GEMM 数据流水线把这些部件串起来, 看清 overlap 如何让多个引擎同时工作.
:::

写出高性能 GPU kernel, 先要理解线程如何组织、数据放在哪里, 以及不同硬件引擎如何协同工作. 本章依次介绍线程层级、保存和搬运数据的内存空间, 以及负责计算和数据搬运的硬件引擎. 随后用一条 GEMM 流水线把这些概念串起来, 说明数据如何在内存空间之间移动, 计算又如何与数据搬运重叠. 后续章节中的许多优化都建立在这些机制之上.
...
Blackwell 上的操作不一定由同一组线程发起. TMA copy 由单个 thread 发起, 再交给硬件执行; 每个 warp 通过 warp-level TMEM load 读取自己的 32-lane window; `tcgen05` MMA 由一个选定的 thread 提交; 2-CTA cooperative MMA 则跨越两个 CTA.
