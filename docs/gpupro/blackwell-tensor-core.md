...
这两种 layout 位于不同的 memory spaces, 具体映射由当前 `tcgen05.mma` 的 instruction shape, 数据类型和 operand major mode 决定. 本章沿用“[数据布局及其记号](/gpupro/data-layout/)”一章介绍的命名轴和 swizzle 概念来描述这些映射.
...
MXFP8 和 NVFP4 是前文介绍的两种具体 block-scaled 格式.<a href="/gpupro/data-layout/#tmem-scale-factors-warp">数据布局章节</a>已经介绍了 block scaling 的计算方式, 以及 SFA, SFB 在 TMEM 中的 packing 和跨 warp replication;<a href="/gpupro/tensor-core-data-layouts/#block-scaled-mma-scale-factors">Tensor Core 数据布局的演进</a>进一步说明了 `tcgen05.cp` 数据路径和 `scale_vec` 的 byte selection. 这里不再重复这些细节, 只看 scale factors 在 CTA pair 中如何放置.
