...
`A[m0:m0+BM, 0:BK]` 取得 `BufferRegion`, `A.ptr_to([i, j])` 取得
元素 pointer, `A.data` 取得原始 data pointer.
...
| `layout` | 物理映射, 详见 [TIRx Layout API](/gpupro/tirx-layout-api/); `"default"` 表示 dense row-major |
...
| `"local"` | `T.alloc_local` | per-thread 寄存器|
...
per-thread 临时数据位于寄存器中. 使用
`T.alloc_local(shape, dtype)` (即 `scope="local"`) 分配; 它只属于当前
thread, 并会生成保存在寄存器中的 local array.
...
寄存器中的 array, 这个 alignment **不会影响性能**: 只要 index 能在
编译期解析, nvcc/ptxas 就会通过 scalar replacement of aggregates
(SROA) 将 thread-local array 提升为寄存器, 因而不会使用可寻址的
local memory. 只有使用动态 index 并 spill 到 local memory 的 array
才会实际受到 over-alignment 影响, 这种情况并不常见.寄存器 local 的
...
Scalar 本质上是只有 **一个元素** 的寄存器 array. 可以直接分配大小为 1
...
`phase += 1` 解析为 `phase[0]` 和 `phase[0] += 1`. 对两个 kernel
...
| `B.local(*shape, layout=…)` | `local` buffer 中属于当前 thread 的 private 寄存器 slice |
...
**寄存器: `local`.** 对带 thread axis 的 `local` layout 做分解, 返回
属于当前 thread 的 flat 寄存器 bundle. tile primitives 会频繁使用它:
