...
前几章已经从不同角度介绍过 TMEM. [数据布局及其记号](/gpupro/data-layout/) 解释了 `TLane`、`TCol` 和二维 layout; [Tensor Core 数据布局的演进](/gpupro/tensor-core-data-layouts/) 介绍了 accumulator 与 scale factors 的数据路径; [Blackwell Tensor Core](/gpupro/blackwell-tensor-core/) 则说明了 `tcgen05.mma` 如何把结果映射到 TMEM.
...
![`tcgen05.ld` 将 TMEM 数据加载到 register fragment,`tcgen05.st` 按相反方向写回](./images/tcgen05_ldst.svg)
...
这和 MMA 的 `M×N×K` instruction shape 是两套概念. MMA shape 描述矩阵乘法的逻辑尺寸; 这里的 data-movement shape 描述 TMEM 与 registers 之间一次搬运的硬件模式. [Tensor Core 数据布局的演进](/gpupro/tensor-core-data-layouts/) 中的 register fragment, 则说明这些 registers 如何对应回逻辑矩阵元素.
...
SMEM 到 TMEM 的 `tcgen05.cp` 使用另一套 shape 和完成机制. 它如何搬运 block-scaled MMA 的 scale factors, 已经在 [Tensor Core 数据布局的演进](/gpupro/tensor-core-data-layouts/) 和 [Blackwell Tensor Core](/gpupro/blackwell-tensor-core/) 中介绍, 这里不再重复.
