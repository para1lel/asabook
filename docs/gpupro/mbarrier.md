...
- 在多级 pipeline 中, 每个阶段通常使用 `full` barrier 通知数据已经就绪, 使用 `empty` barrier 归还 buffer. barrier 可以通过 phase 反复使用, kernel 需要跟踪 phase parity, 避免把上一轮的完成状态误认为当前轮已经完成.
:::
...
每次 arrival 都会减少 barrier 当前仍在等待的工作负载. kernel 中的不同参与者会以不同方式发出 arrival.
...
关键在于, 异步硬件不仅在后台继续执行, 还会通过 barrier 报告完成. TMA 可以通知 shared-memory tile 已经准备好, Tensor Core 可以通知 TMEM 中的结果已经写完, 普通 thread 也可以通知某块 buffer 已经不再使用. 它们最终都遵循同一种 producer - consumer 协议: producer 报告 arrival, consumer 执行 wait.
...
上图展示了一个 barrier 如何在 phase 0 和 phase 1 之间切换, 从而被反复使用. 在两级 TMA pipeline 中,阶段 0 和阶段 1 各有一块 SMEM buffer, 也各有一个 TMA barrier. 这样, consumer 可以分别等待每个阶段的数据.

当两个阶段按阶段 0,阶段 1 的顺序循环访问时, 可以用一个 `phase_tma` 记录当前轮次. 每访问完两个阶段, 就翻转一次 parity:
...
两个 barrier 初始化后都处于 phase 0, `phase_tma` 也从 0 开始. 前四次 iterations 的完整状态变化如下:

| iteration |阶段| 本轮等待的 phase parity | 该 barrier 完成后的当前 parity | iteration 结束后的 `phase_tma` |
|---: |---: |---: |---: |---: |
| 0 | 0 | 0 | 1 | 0 |
| 1 | 1 | 0 | 1 | 1 |
| 2 | 0 | 1 | 0 | 1 |
...
Iteration 0 等待阶段 0 的 phase 0. 该 phase 完成后,阶段 0 的 barrier 进入 phase 1, 但这一轮 circular buffer 还没有遍历完, 所以 `phase_tma` 仍为 0. Iteration 1 等待阶段 1 的 phase 0; 等待完两个阶段后, `phase_tma` 翻转为 1.

Iteration 2 回到阶段 0, 这次等待 phase 1; 完成后,阶段 0 的 barrier 回到 phase 0. Iteration 3 同样等待阶段 1 的 phase 1. 第二轮两个阶段都完成后, `phase_tma` 也翻转回 0.

`phase_tma` 描述的是软件访问 circular buffer 的轮次, 与两次 TMA 传输在硬件上谁先完成无关. 因此, 深度为 `S` 的 TMA pipeline 通常为每个阶段准备一个记录 TMA 完成状态的 `full` barrier, 再通过 phase parity 区分同一个阶段的前后两轮. 完整的 buffer 复用协议还需要下文介绍的 `empty` barrier.

## 常见的同步规则

在 Tensor Core kernel 中, `mbarrier` 主要用于协调以下三种数据交接.

**thread 向异步硬件交接数据.** 如果 thread 先写入 shared memory, 随后由 TMA store 或 MMA 读取这块数据, kernel 必须先完成相应的同步和顺序保证. 否则, 异步操作可能在 shared-memory buffer 尚未写完时就开始读取.
...
barrier 不仅可以表示“数据已经准备好”, 也可以表示“buffer 已经用完”. 因此, pipelined kernel 通常会为每个 SMEM 阶段准备一对 barrier: `full[stage]` 表示 TMA 已经填好该阶段, `empty[stage]` 表示 consumer 已经用完这块 buffer. 在 pipeline 稳定运行时, 一个阶段的循环可以概括为:

![一个 SMEM 阶段的 full/empty barrier 复用协议](./images/mbarrier_stage_reuse_v2_zh.svg)

`full` 负责从 producer 向 consumer 交接数据, `empty` 负责从 consumer 向 producer 归还 buffer. 每个 barrier 的 expected arrival count 取决于有多少个 thread 负责报告完成. pipeline 循环复用同一对 barrier 时, 还需要分别跟踪 `full` 和 `empty` 的 phase parity.
