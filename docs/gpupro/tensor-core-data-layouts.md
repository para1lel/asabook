...
下面我们将按照 Ampere, Hopper, Blackwell 的顺序, 追踪三代架构中的数据路径, 并分别考察三个相同的问题: MMA 从哪里读取输入? 累加结果保存在哪里? kernel 如何描述指令要求的 layout? 描述具体映射时, 我们会继续使用“[数据布局及其表示方法](/gpupro/data-layout/)”一章引入的记号.
...
一条 `mma.sync` 由整个 warp 协同执行. A, B 和 C/D 矩阵 tile 会按照 PTX 规定的方式, 分散到 warp 中 32 个 thread 的寄存器里. 每个 thread 只持有 tile 的一部分, 这一部分称为该 thread 的寄存器 fragment; 把 32 个 thread 持有的部分合在一起, 才得到完整的矩阵 tile.
...
接下来以 `mma.sync.aligned.m16n8k16` 为例, 看看一个矩阵 tile 的元素如何分配到 warp 中 32 个 thread 的寄存器里. 这里令 A 为 row-major, B 为 column-major, A/B 使用 `fp16` 或 `bf16`, C/D 使用 `fp32`. 整个 warp 共同计算:
...
观察 PTX 规定的映射, 可以发现它每 4 个 lane 重复一次: lane 0–3 负责第 0 行和第 8 行, lane 4–7 负责第 1 行和第 9 行, 后面的 lane 依次类推. 因此, 可以把 32 个 lane 分成 8 个连续的 4- lane groups. 记当前 lane 的 ID 为 $l$, 并定义:
...
A fragment 使用相同的 $g$ 和 $t$, 但此时坐标表示的是 $(m,k)$. 每个 lane 持有 8 个 `fp16` 或 `bf16` 元素, 每两个元素打包在一个 32-bit 寄存器中:

$$
\begin{aligned}
\text{寄存器 0}: &\ (g, \ 2t+\{0,1\}), \\
\text{寄存器 1}: &\ (g+8, \ 2t+\{0,1\}), \\
\text{寄存器 2}: &\ (g, \ 2t+\{8,9\}), \\
\text{寄存器 3}: &\ (g+8, \ 2t+\{8,9\}).
\end{aligned}
$$

换成矩阵坐标来看, 该 lane 在 $m=g$ 和 $m=g+8$ 两行中, 各持有 K 低半区和高半区的一对相邻元素.

B fragment 的坐标表示为 $(k,n)$. 每个 lane 持有 4 个 `fp16` 或 `bf16` 元素, 同样每两个元素打包在一个 32-bit 寄存器中:
...
到了 B fragment, $g$ 决定 $n$ 坐标, $t$ 与寄存器编号共同决定 $k$ 坐标.
...
上面已经知道 `mma.sync` 需要什么样的寄存器 fragment. 接下来的问题是: A, B 已经放在 shared memory 中, 怎样一次把它们送到正确的 thread 寄存器? Ampere 为此提供了 `ldmatrix`. 下面讨论它的 `.m8n8.b16` 形式:
...
这组指令由整个 warp 协同执行. `.x1`, `.x2` 和 `.x4` 分别加载 1,2 和 4 个 `8×8` 矩阵块. 对于第 `m` 个矩阵块的第 `r` 行, 起始地址由 lane `m·8+r` 提供. 因此, `.x1` 使用 lane 0–7 提供地址, `.x2` 使用 lane 0–15, `.x4` 使用全部 32 个 lane.

下图展示的是 `.x1`. 图左侧的 T0–T7 分别提供八行的起始地址, 但“提供地址”和“接收数据”是两回事: 加载得到的 64 个元素会分发给全部 32 个 thread. lane $l$ 接收:
...
例如, 第 0 行的八个元素会分配给 lane 0–3: lane 0 接收 columns 0–1, lane 1 接收 columns 2–3, 以此类推. 每个 lane 接收的两个 `fp16` 元素会打包到一个 32-bit 寄存器中. 图左侧画的是地址由谁提供, 图右侧画的是数据最终由谁持有. 反向箭头表示 Hopper (`sm_90`) 加入的 `stmatrix`, 它把寄存器 fragment 写回 shared memory.

可选的 `.trans` 修饰符会以 column-major 方式加载每个 `8×8` 矩阵块. 如果不使用 `ldmatrix`, kernel 也可以通过普通 loads 和寄存器操作构造 fragment, 但需要自己完成这套跨 lane 的数据分发.

![ldmatrix 将一个 8×8 shared memory tile 加载到 warp 寄存器 fragment; 图中的反向 stmatrix 路径仅适用于 Hopper sm_90 及后续架构](./images/ldstmatrix.svg)

### 写回结果与对 shared memory 做 swizzle

`mma.sync` 完成后, C/D 仍然保存在寄存器 fragment 中. 在 Ampere 上, epilogue 通常由各 thread 执行普通 stores, 将结果写入 shared memory 或 global memory, 必要时再配合 warp shuffles 或局部重排.
...
到了 Hopper, MMA 的协作范围从一个 warp 扩大到一个 warpgroup. 一个 warpgroup 由 4 个连续的 warp 组成, 也就是 128 个 thread; 它们共同执行 `wgmma.mma_async`.
...
当输入来自 shared memory 时, kernel 无需再用 `ldmatrix` 构造 A/B 寄存器 fragment. WGMMA 会直接读取数据, 但它仍然需要知道矩阵从哪里开始, 跨到下一组数据时移动多少字节, 以及 shared memory 使用了哪种 swizzle. 这些信息都放在 matrix descriptor 中.
...
WGMMA 直接读取 shared memory 中的输入, 计算得到的 C/D 仍然分散在 warpgroup 内各 thread 的寄存器中. epilogue 随后处理这些寄存器 fragments. 每个 thread 持有多少个累加值, 由 WGMMA 的 instruction shape 和累加类型决定.

于是, Hopper kernel 中会同时看到两种布局表示: shared memory 中的 A/B 用 matrix descriptor 描述,寄存器中的 A 和 C/D 则采用 per-thread 寄存器 fragment. B 一直来自 shared memory, A 可以选择其中一条路径.
...
上一章的 <a href="/gpupro/data-layout/#tmem-中-scale-factors-的跨-warp-广播">TMEM 中 Scale Factors 的跨 warp 广播</a> 已经用 `M=128`, `SFK=4` 的例子, 推导了 scale factors 在 TMEM 中的打包和 `.warpx4` replication. 现在沿着数据路径继续往下看: 这些值怎样进入 TMEM, MMA 又怎样选中当前需要的几个 bytes.
...
## 三代架构中的 per-thread 寄存器 Fragment

把三代架构放在一起看, 矩阵数据都会在某些阶段以寄存器 fragment 的形式分布到各 thread 的寄存器中. Fragment 的具体 shape 和映射由相应指令决定.

在 Ampere 上, `ldmatrix` 从 shared memory 加载数据, 并构造 `mma.sync` 读取的寄存器 fragment.

在 Hopper 上, `wgmma` 将累加结果写入寄存器 fragment, 交给 epilogue 继续处理.

到了 Blackwell, 计算期间的累加器存放在 TMEM 中; epilogue 开始前, `tcgen05.ld` 再把结果加载成寄存器 fragment.

因此,寄存器 fragment 在三代架构中承担着不同角色. Ampere 和 Hopper 在计算期间用它保存累加器; Blackwell 则主要在 TMEM 与 epilogue 的交界处使用它. 前面介绍的 m8n8-style atom 是一个常见例子; `tcgen05.ld` 和 `tcgen05.st` 还支持其他 data-movement shapes.

## 三种数据路径的对比

| 架构 | 主要 MMA 指令 | A/B 主要来源 | 累加器位置 | Shared-memory layout 如何表达 |
| --- | --- | --- | --- | --- |
| Ampere | `mma.sync` |寄存器|寄存器| kernel 通常手写地址计算和 swizzle |
| Hopper | `wgmma.mma_async` | A 可来自寄存器或 SMEM; B 来自 SMEM |寄存器| matrix descriptor 记录 strides 和 swizzle mode |
| Blackwell | `tcgen05.mma` | 主要来自 SMEM; 某些 A 模式可来自 TMEM | TMEM | descriptor 描述 SMEM 输入, TMEM layout 另行描述累加器和 scale factors |

三代对照来看, 这条演进脉络就很清晰了: Ampere 先把输入整理成 per- lane 寄存器 fragments; Hopper 让 Tensor Core 通过 descriptor 直接读取 shared memory; Blackwell 再把累加器和 scale factors 移入 TMEM.
