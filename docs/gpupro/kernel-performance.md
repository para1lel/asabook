...
- 高算术强度通常意味着 compute-bound, 也就是性能主要受计算吞吐限制. 优化重点是让 Tensor Core 保持忙碌, 并通过重叠执行 (overlap) 减少计算路径上的空闲时间.
:::
...
下面用一个简化模型说明这种复用. 假设一个 CTA 计算 `B_M × B_N` 的 C tile; 每个 K-阶段加载 `B_M × B_K` 的 A tile 和 `B_K × B_N` 的 B tile; 每个元素占 `s` bytes. 只统计这个阶段的 A/B global memory 访问, 不考虑 C 的读写, 算术强度近似为:
...
举个带数字的例子. 这里整个 GEMM 的矩阵尺寸 `M`, `N`, `K` 不变, 总计算量 `2MNK` 也不变; 我们只是在比较同一个 GEMM 下不同 CTA tile 大小带来的片上复用差异. 假设数据类型是 FP16/BF16, 则 `s = 2 bytes`, 并且每个 K-阶段取 `B_K = 64`. 如果 CTA tile 是 `16 × 16`, 这一阶段的计算量是:
...
所以这一阶段的 A/B global-memory 算术强度是 `32768 / 4096 = 8 FLOP/byte`. 如果把 CTA tile 改成 `64 × 64`, 仍然使用 `B_K = 64`, 计算量变成:
...
实际 kernel 还会受到 C 的读写, L2 reuse, occupancy,寄存器 pressure, SMEM bandwidth 和 Tensor Core issue rate 等因素影响. 这里的模型虽然简化, 但已经说明了 tiling 提高算术强度的原因: 同一个 A/B 元素从 global memory 读入后, 可以在 tile 内参与更多次乘加.
...
在 GEMM 的优化阶梯中, 第一个明显的实测性能跃升, 是从 thread -copy tiled 路径切换到 TMA-backed 路径. 前者由 CTA 中的普通线程执行 GMEM 到 SMEM 的拷贝; 后者把这种规则的 tile 搬运交给 TMA 硬件引擎, 让 kernel 可以通过硬件管理的大块拷贝来持续为 Tensor Core 提供数据.
...
一旦 GEMM 已经是 compute-bound 的, 并且已经使用了 Tensor Core, 剩下的性能差距通常来自于硬件的空闲时间.
...
SM 占用率受寄存器, shared memory, warp slots 和 CTA slots 的限制. warp slots 和 CTA slots 分别规定一个 SM 能同时容纳的 warp 数和 CTA 数. 如果每个 thread 使用很多寄存器, 或者每个 CTA 使用大量 shared memory, 一个 SM 能驻留的 CTAs 或 warp 就会减少, occupancy 也会下降.

许多现代 Tensor Core kernel 会主动消耗更多资源, 即使这会降低 occupancy. 多阶段的 shared memory pipeline 会占用 SMEM; 较大的寄存器 fragments 会占用寄存器; TMEM allocation 会占用 Tensor Memory 容量; warp specialization 也可能把整组 warp 固定分配给 producer 或 consumer 角色.
