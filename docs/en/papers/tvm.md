...
Latency hiding refers to the process of overlapping memory operations with computation to maximize utilization of memory and compute resources. It requires different strategies depending on the target hardware back-end. On CPUs, memory latency hiding is achieved implicitly with simultaneous multithreading [Sept97] or hardware prefetching [Improa90, May95]. GPUs rely on rapid context switching of many warps of threads [Berkea16]. In contrast, specialized DL accelerators such as the TPU [USAa17] usually favor leaner control with a *decoupled access-execute* (DAE) architecture [USA82] and offload the problem of fine-grained synchronization to software.
...
One way to find the best schedule from a large configuration space is through blackbox optimization, i.e., auto-tuning. This method is used to tune high performance computing libraries[Automa98, Procea98]. However, auto-tuning requires many experiments to identify a good configuration.
