...
In the core attention of CSA and HCA, we employ the trick of attention sink [Xia24a, Ope25c]. To be specific, we set a series of learnable sink logits $\{z^{\prime}_{1},z^{\prime}_{2},...,z^{\prime}_{n_{h}}\}$. For the $h$-th attention head, $\mathrm{Exp}(z^{\prime}_{h})$ will be added to the denominator of the attention score:
...
The attention architecture of DeepSeek-V4 series allows us to directly apply RMSNorm on the attention queries and KV entries, which effectively prevents attention logits from exploding. Consequently, we do not employ the QK-Clip technique [Liu25] in our Muon optimizer.
...
**Figure 5.** Illustration of our EP scheme with related works. Comet [Zha25d] overlaps Dispatch with Linear-1, and Linear-2 with Combine, separately. Our EP scheme achieves a finer-grained overlapping by splitting and scheduling experts into waves. The theoretical speedup is evaluated in the configuration of the DeepSeek-V4-Flash architecture.
...
**World knowledge** benchmarks include AGIEval [Zho23], C-Eval [Hua23], CMMLU [Li23e] MMLU [Hen20], MMLU-Redux [Gem24], MMLU-Pro [Wan24c], MMMLU [Ope24c], MultiLoKo [Hup25], Simple-QA verified [Haa25], SuperGPQA [Du25a], FACTS Parametric [Che25], and TriviaQA [Jos17].
...
Knowledge and reasoning datasets include MMLU-Pro [Wan24c], GPQA [Rei24], Human Last Exam [Pha25], Simple-QA Verified [Haa25], Chinese-SimpleQA [He24], LiveCodeBench-v6 [Jai24], CodeForces (Internal Benchmark), HMMT 2026 Feb, Apex [Bal25], Apex Shortlist [Bal25], IMOAnswerBench [Luo25], and PutnamBench [Tso24].
...
Agent datasets include Terminal Bench 2.0 [Mer26], SWE-Verified [Ope24e], SWE Multilingual [Yan25b], SWE-Pro [Den25], BrowseComp [Wei25], the public evaluation set of MCPAtlas [Ban26a], GDPval-AA [Pat25, Aa25], and Tool-Decathlon [Li25b].
