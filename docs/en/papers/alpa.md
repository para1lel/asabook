...
DL computation is commonly represented by popular ML frameworks [OSDIb16, Systel19, NumPyb18] as a dataflow graph. Edges in the graph represent multi-dimensional tensors; nodes are computational operators, such as matrix multiplication (matmul), that transform input tensors into output tensors. Training a DL model for one iteration consists of computing a loss by *forwarding* a batch of data through the graph, deriving the updates via a reverse *backward* pass, and applying the updates to the parameters via *weight update* operations. In practice, model developers define the dataflow graph. An execution engine then optimizes and executes it on a compute device.
...
[Fig. 2d](#figure-02) illustrates the batch-splitting pipeline parallelism as a case of inter-operator parallelism. [+2] The pipeline execution can follow different schedules, such as Gpipe [Advanb19], PipeDream [Prince19], and synchronous 1F1B [PMLRb21, Progrc21]. We adopt the synchronous 1F1B schedule throughout this paper as it respects synchronous consistency, and has the same pipeline latency but lower peak memory usage compared to Gpipe.
...
For other primitive operators such as convolution and reduction, we can get a list of possible parallel algorithms following a similar analysis of their math expressions. In the intra-op pass, the model graph is represented in XLA’s HLO format[Optimi17], which summarizes common DL operators into less than 80 primitive operators, so we can manually enumerate the possible parallel algorithms for every primitive operator.

### 4.2 ILP Formulation

The total execution cost of a computational graph $G=(V,E)$ is the sum of the compute and communication costs on all nodes $v\in V$ and the resharding costs on all edges $e\in E$. We formulate the cost minimization as an ILP and solve it optimally with an off-the-shelf solver [INFORM05].
...
After stages, device meshes, and their assignments are decided, at the intra-op level, Alpa compiles each stage against its assigned device mesh, respecting the intra-op parallelism plan output by the ILP solver. The compilation depends on XLA [Optimi17] and GSPMD [Xivan21], and generates parallel executables for each stage-mesh pair. When needed, the compilation automatically inserts collective communication primitives (see §[4](#S4 "4 Intra-Operator Parallelism ‣ Alpa: Automating Inter- and Intra-Operator Parallelism for Distributed Deep Learning")) to address the *within-mesh* communication caused by intra-op parallelism.
...
Compared to existing work that manually combines data, operator, and pipeline parallelism, such as 3D parallelism [Mining20] and PTD-P [Analys21], Alpa’s hierarchical view of inter- and intra-op parallelisms significantly advances them with three major flexibility: (1) pipeline stages can contain an uneven number of operators or layers; (2) pipeline stages in Alpa might be mapped to device meshes with different shapes; (3) within each stage, the data and operator parallelism configuration is customized non-uniformly on an operator-by-operator basis. Together, they allow Alpa to unify all existing model parallelism approaches and generalize to model architectures and cluster setups with more heterogeneity.
