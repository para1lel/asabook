...
The emergence of large language models (*LLMs*) like GPT [OpenAa23, Browne20] and PaLM [Chowdb22] have enabled new applications such as programming assistants [Githua00, Chena21] and universal chatbots [Blog22, Bard23] that are starting to profoundly impact our work and daily routines. Many cloud companies [OpenAI20, Servic23] are racing to provide these applications as hosted services. However, running these applications is very expensive, requiring a large number of hardware accelerators such as GPUs. According to recent estimates, processing an LLM request can be 10$\times$ more expensive than a traditional keyword query [Reuter23]. Given these high costs, increasing the throughput—and hence reducing the cost per request—of *LLM serving* systems is becoming more important.
...
Once trained, LLMs are often deployed as a conditional generation service (e.g., completion API [OpenAI20] or chatbot [Blog22, Bard23]). A request to an LLM service provides a list of *input prompt* tokens $(x_{1},\ldots,x_{n}),$ and the LLM service generates a list of output tokens $(x_{n+1},\ldots,x_{n+T})$ according to Eq. [1](#S2.E1 "Equation 1 ‣ 2.1. Transformer-Based Large Language Models ‣ 2. Background ‣ Efficient Memory Management for Large Language Model Serving with PagedAttention"). We refer to the concatenation of the prompt and output lists as *sequence*.
...
Since most operators in current deep learning frameworks [Paszka19, Olston17] require tensors to be stored in contiguous memory, previous LLM serving systems [Refd22, Fastea00] also store the KV cache of one request as a contiguous tensor across the different positions. Due to the unpredictable output lengths from the LLM, they statically allocate a chunk of memory for a request based on the request’s maximum possible sequence length, irrespective of the actual input or eventual output length of the request.
...
Specialized serving systems for transformers. Due to the significance of the transformer architecture, numerous specialized serving systems for it have been developed. These systems utilize GPU kernel optimizations [Wang21, Aminab22, Fastea00, Refc20], advanced batching mechanisms [Fang21, Refd22], model parallelism [Popea22, Refd22, Aminab22], and parameter sharing [Zhou22] for efficient serving. Among them, Orca [Refd22] is most relevant to our approach.
