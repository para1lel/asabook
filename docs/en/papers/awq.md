...
Large language models (LLMs) based on transformers [Advand17] have shown excellent performance on various benchmarks [Systea01, Open22, Touvrc23, Xivr22]. However, the large model size leads to the high serving costs. For example, GPT-3 has 175B parameters, which is 350GB in FP16, while the latest H100 GPU only has 96GB memory, let alone edge devices.
...
Our primary baseline is vanilla round-to-nearest quantization (RTN). It is actually quite strong when using a small group size like 128 [Xivo22, Xivn22]. We also compare with a state-of-the-art method GPTQ [Xivo22] for LLM weight quantization. For GPTQ, we also compare with an updated version that uses a “reorder” trick (denoted as GPTQ-Reorder or GPTQ-R). Other techniques like ZeroQuant [Effici22], AdaRound [PMLRa20], and BRECQ [Xivaw21] rely on backpropagation to update the quantized weights, which may not easily scale up to large model sizes; they also do not outperform GPTQ [Xivo22], thus not included for study.

### 3.2 Evaluation

#### Results on LLaMA models.

We focus our study on LLaMA models (LLaMA [Touvrc23] and Llama-2 [Xivf23]) due to their superior performance compared to other open-source LLMs [Open22, Xivr22]; it is also the foundation of many popular open-source models [Stanfa23, Marcha23]. We evaluate the perplexity before and after quantization in [Table 4](#table-04). We can see that AWQ consistently outperforms round-to-nearest (RTN) and GPTQ [Xivo22] (w/ and w/o reordering) across different model scales (7B-70B) and generations.
