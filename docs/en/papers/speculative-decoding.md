...
Another type of approximation models that can be used by speculative decoding are non-autoregressive models, like those from [Ste18]. Then, instead of the autogreressive loop in [Algorithm 1](#alg1 "In 2.3 Speculative Sampling ‣ 2 Speculative Decoding ‣ Fast Inference from Transformers via Speculative Decoding") we’d just call the non-autoregressive model once.
...
We tested a decoder only LaMDA model on a dialog task [Tho22]. We used existing checkpoints from LaMDA 137B as $M_{p}$ and LaMDA 8B, LaMDA 2B, and LaMDA 100M for $M_{q}$.
