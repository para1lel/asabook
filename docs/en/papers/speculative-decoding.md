...
**Definition 3.1.** The *acceptance rate $\beta_{x_{<t}}$*, given a prefix $x_{<t}$, is the probability of accepting $x_{t}\sim q(x_{t}|x_{<t})$ by speculative sampling, as per [Section 2.3](#section-02-03) [+2].
...
**Definition 3.2.** $D_{\mathrm{LK}}(p,q)=\sum_{x}|p(x)-M(x)|=\sum_{x}|q(x)-M(x)|$ where $M(x)=\frac{p(x)+q(x)}{2}$.

<span id="lemma-03-03"></span>

**Lemma 3.3.** $D_{\mathrm{LK}}(p,q)=1-\sum_{x}\min(p(x),q(x))$
...
**Corollary 3.4.** $D_{\mathrm{LK}}(p,q)$ is a symmetric divergence in $[0,1]$.<br>
$D_{\mathrm{LK}}(p,q)=0\iff p=q$.<br>
$D_{\mathrm{LK}}(p,q)=1\iff p$ and $q$ have disjoint support.

<span id="theorem-03-05"></span>

**Theorem 3.5.** $\beta=1-D_{\mathrm{LK}}(p,q)$
...
**Corollary 3.6.** $\alpha=1-E(D_{\mathrm{LK}}(p,q))=E(\min(p,q))$
...
**Definition 3.7.** Let $c$, the *cost coefficient*, be the ratio between the time for a single run of $M_{q}$ and the time for a single run of $M_{p}$.
...
**Theorem 3.8.** The expected improvement factor in total walltime by [Algorithm 1](#algorithm-01) is $\frac{1-\alpha^{\gamma+1}}{(1-\alpha)({\gamma}c+1)}$.
...
**Definition 3.10.** Let $\hat{c}$ be the ratio of arithmetic operations per token of the approximation model $M_{q}$ to that of the target model $M_{p}$.

**Theorem 3.11.** The expected factor of increase in the number of total operations of [Algorithm 1](#algorithm-01) is $\frac{(1-\alpha)({\gamma}\hat{c}+\gamma+1)}{1-\alpha^{\gamma+1}}$.
