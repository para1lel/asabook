...
This is the first of three chapters that walk a single GEMM optimization path from end to end. In this one we build a correct tiled kernel and stop there. The next chapter ([Pipelining GEMM with TMA](/en/gpupro/pipelined-gemm/)) replaces the thread copies with TMA and overlaps data movement with compute through pipelining, and [Scaling GEMM with Warp Specialization and Clusters](/en/gpupro/warp-specialized-gemm/) goes further still with warp specialization and CTA clusters. Each chapter builds on the one before it, so the kernels accumulate features rather than start over.
